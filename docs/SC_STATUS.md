...
2. **PDF schedule export** for overview + drill-down. **SHIPPED**: Wave 1 (#419, Month + Period + Season sheets) + Wave 2 (#420, Year sheet). All four sheets faithful to `docs/design/SC_PRINT_SPEC_v1.html`. Serverless headless Chrome via `puppeteer-core` + `@sparticuz/chromium`; fonts self-hosted; timezone per-account via `ACCOUNT_HOME_TZ`. Export menu: drill-in gets Excel + PDF this scope + Excel year fallback; overview gets Excel + PDF season (schedule accounts only) + PDF year at a glance (all accounts).
