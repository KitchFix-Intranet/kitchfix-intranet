...
2. **PDF schedule export** for overview + drill-down. **SHIPPED**: Wave 1 (#419) + Wave 2 (#420) + Wave 3 v2 restyle (#422). Four sheets now faithful to `docs/design/SC_PRINT_SPEC_v2.html` (`v1` kept for history). v2 introduces the SERVED / PROJECTED / NO ACTUALS / NO SERVICE state model, retires the year sparkline in favor of the Ops Calendar (compliance surface with period-start navy squares, spring bars, M/F header chips), and adds AWAY cells + day numbers to full-schedule seasons plus a blended service-calendar variant for overlay PDCs. Serverless headless Chrome via `puppeteer-core` + `@sparticuz/chromium`; fonts self-hosted; timezone per-account via `ACCOUNT_HOME_TZ`. Export menu: drill-in gets Excel + PDF this scope + Excel year fallback; overview gets Excel + PDF season (schedule accounts only, blended variant for overlay accounts) + PDF ops calendar (all accounts; label renamed from "year at a glance").

- **Follow-up standalone PR (migration-gated)**: `period_data` → PG migration + inventory-due copper ring wiring on the Ops Calendar. `period_data` currently lives in Sheets HUB (read by `/api/cron/daily/route.js`); the migration collapses the duplication with `sc_day_metadata.period` and lets the print route + the daily-inventory-notifications cron + Smart Inventory v2 substrate all share one PG source. Ops Calendar's copper open-ring glyph waits for this migration; until then no ring AND no legend entry (legend matches reality).
