...
- **`PriceEditPanel.js` + `FeeEditPanel.js`** - two-phase Save. Save-click on Backdate triggers the preview endpoint. If closed periods are returned, an amber confirmation modal names the periods, the day count, and the revenue delta (price only), and the operator must confirm before the write posts. Today and Future radios never trigger the preview - by construction they cannot reach a closed period.
