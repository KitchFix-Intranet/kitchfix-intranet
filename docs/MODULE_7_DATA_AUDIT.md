...
- **`inventory_items.last_price` / `last_price_date` / `last_price_vendor` → DERIVED, NOT STORED (INV-1 review delta).** Same drift-elimination philosophy as the count-totals choice above: `price_history` is the authoritative ledger, so a stored `last_price` cache re-introduces the "two values that must agree" failure mode the migration just retired. The bootstrap view `v_inventory_items_full` derives them from the latest `price_history` row per item (keyed on `recorded_at DESC` so backfilled historical rows can't supersede live cron writes). Supporting index: `price_history (item_id, recorded_at DESC)`. `handleVerifyPrice` and the cron's catalog-write path collapse to a single `INSERT INTO price_history`; no second column to update.
- **`inventory_items.category` is an ENUM (`Food`/`Packaging`/`Supplies`/`Snacks`/`Beverages`), not TEXT (INV-1 review delta).** The totals view matches by exact category string; a typo or stray value would silently misbucket money. Verified against the 3,666 live `item_catalog` rows on 2026-06-04: zero strays. Enum makes the silent-misbucket failure mode impossible.
- **`merge_history.action` enum keeps the 7 historical values (INV-1 review delta).** Not collapsed: `archive` (reversible) and `exclude` (never re-import) are semantically distinct - the cron honors them differently - and lossy-mapping `archive → exclude` would destroy information. Schema models what actually happens: `'merge', 'keep_separate', 'link', 'exclude', 'archive', 'reactivate', 'review_delete'`. Whether those handlers SHOULD continue writing to merge_history is an INV-2 question; INV-1 accommodates the real data.
