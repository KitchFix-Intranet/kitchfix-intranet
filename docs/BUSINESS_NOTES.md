...
- **CIN-AZ hybrid: per-meal PLUS a real contract service fee** (Kevin ruling 2026-07-12). CIN-AZ (`actuals_drive_invoice`) bills per-meal actuals AND a separate annual service fee. The two are additive P&L lines per [`SC_MONEY_MODEL.md`](SC_MONEY_MODEL.md); per-meal actuals continue to drive the per-meal invoice, and the contract fee lives in `sc_fee_schedule` alongside the flat-fee accounts. Admin surface visibility widened via `FEE_ELIGIBLE_PER_MEAL = ["CIN - AZ"]` in `src/lib/dataStore/serviceCalendar.js` (PR #417); calendar tile shape and classifier stay per-meal.
...
### Directory SA migration (Bundle 3 PR B2)

- **What:** `src/app/api/directory/route.js` fully migrated from user-OAuth to canonical SA. 13 read sites converted to `readSheetSA`, 13 writes converted to canonical helpers (`updateRangeSA`, `updateCellSA`, `appendRowSA`, `appendRowsSA`, `clearRangeSA`, `deleteRowSA` + `getSheetIdSA`), 2 Drive proxy calls converted to `getServiceAccountDriveClient()`. Three local read helpers removed (`safeReadSheet`, inline `safeRead` closure, `getSheetTabId`). Two user-OAuth client constructors removed (`getSheetsClient`, `getDriveClient`). The `import { google } from "googleapis"` dropped (no remaining `google.*` symbolic references; only `"drive.google.com"` string literal in `getDriveFileId` remains). Three helper functions (`writeLinks`, `upsertWorkLocation`, `removeWorkLocation`) dropped their `sheets` + `token` params at both definitions and all 6 call sites. File 548 -> 434 LOC (-114).
- **Why:** Completes the user-OAuth-to-SA consolidation for directory. The original PR B refactor scope, executed after B1 carved out the security gate. directory is now Stage-1-ready: swap the canonical helpers (sheets.js + gmail.js) to Postgres-backed equivalents and directory's 28 operations follow automatically with zero per-call-site edits.
- **Where:** `src/app/api/directory/route.js` (the only file touched in B2's code surface).
- **D.3a Drive decision + empirical verification:** The Drive proxy migration was gated on a 2-minute throwaway test (now deleted from the tree). `getServiceAccountDriveClient().files.get` was invoked on every production Drive URL in the HUB accounts tab - all 12 succeeded (12/12, all `image/png`, 220-555 KB, all in a shared Drive folder the SA can reach). Critical finding from the test: cols H (img) and I (logo) hold ZERO Drive URLs (only public/CDN URLs that `getDriveFileId` returns null for); the proxy's entire Drive workload is col R (gmapImg). `supportsAllDrives: true` was already present on both production `drive.files.get` calls and matched the test's API call shape exactly. This is why D.3a (full SA migration) was safe vs the fallback D.3b (keep proxy as user-OAuth documented exception).
- **B1 gate relationship:** B1's interim admin gate (PR #57) is untouched by B2. It reads `process.env.DIRECTORY_ADMIN_EMAILS` and `session.user.email`, both independent of the Sheets/Drive auth model. B2 removed the token extraction (`const token = session.accessToken; if (!token) ...`) that sat adjacent to the gate, but the gate block (L226 banner + L236-243 parse + check) is byte-identical to PR #57's ship. Verified by comparing the gate's code lines pre- and post-B2.
- **Shape-adaptation note (silent-failure class):** The two old read helpers returned DIFFERENT shapes - `safeReadSheet` returned a bare array of rows, the inline `safeRead` closure returned `{headers, rows}`. Canonical `readSheetSA` returns `{headers, rows}`. So the 6 bootstrap inline-safeRead callers needed no adaptation (already used `.rows` access), but the 7 module-level safeReadSheet callers needed `{ rows }` destructuring added (`const { rows } = await readSheetSA(...)`). Documented because shape mismatch on a read is silent (undefined rows, empty render, no throw) - the kind of bug that would only surface during user testing.
- **Write-correctness notes (3 traps handled):** (a) Write #8 hero_images is a COLUMN write (`urls.map(u => [u])` = 2D array N rows × 1 col, written to range `hero_images!A1` which extends downward) - NOT a single cell. Migrated to `updateRangeSA(SHEET_IDS.HUB, "hero_images!A1", urls.map(u => [u]))` with an explanatory inline comment. (b) Write #5 contacts is a MULTI-ROW DELETE LOOP - the `.reverse()` on `toDelete` is load-bearing (deletes bottom-up to prevent index shift). Preserved with an explanatory comment. (c) Write #6 contacts is a MULTI-ROW APPEND (`contacts.map(c => [...])` = 2D array) - mapped to `appendRowsSA` (plural), NOT `appendRowSA` (singular). Column widths preserved on all appends (accounts=18 A:R, contacts=6 A:F, dir_links=5 A:E, work_locations=3 A:C); appendRowSA's auto-`!A:A` anchoring handles offset-corruption prevention internally.
- **getSheetTabId caller recount:** The original PR B recon said `getSheetTabId` had 1 caller. The fresh re-read at SP4 found 2 (in admin-update-contacts AND in removeWorkLocation). Both migrated to canonical `getSheetIdSA` together. The recon count was stale relative to the actual file - PR #57 (B1) shifted line numbers between recon and execution. (Counts must be re-anchored at execution time, not trusted from recon.)
- **Cleanup details:** Removed `const sheets = getSheetsClient(token);` (dead after the 13 writes migrated). Removed `const token = session.accessToken; if (!token) ...` extraction + check in both GET and POST handlers (dead after all user-OAuth call sites migrated). Removed both client constructor functions. The `if (!session)` authentication check stays in both handlers (still need authentication; the question changed from "is the user authenticated AND have they granted Drive scope" to just "is the user authenticated").
- **LOC:** -114 (548 -> 434).
- **POST-MERGE VERIFICATION REQUIRED:** Like PR #57 (B1) and PR #56 (Gmail), the write migration's real proof is a live write. **Smoke test the following after merge:**
  - Edit an existing account name + save (exercises writes #1, #11, #9 via admin-update-account path)
  - Add a new account (exercises writes #2, #12, #10 via admin-add-account)
  - Update contacts on an account (exercises writes #5 multi-row delete + #6 multi-row append)
  - Update hero images list (exercises #7 clear + #8 column write)
  - Deactivate an account (exercises #3 single cell + #13 work_locations delete)
  - Reactivate the same account (exercises #4 single cell + #11 work_locations upsert)
  - View a team detail page that renders the gmapImg (exercises the Drive proxy on SA - confirms the 12/12 test result in production)
  - Confirm B1 gate still blocks non-admins: a non-`DIRECTORY_ADMIN_EMAILS` authenticated user hits any admin action via direct API call -> 403
- **Documented:** 2026-05-22 during Bundle 3 PR B2 sub-phase 8.
- **Migration consideration:** Stage 1 swaps the canonical SA helpers (`readSheetSA`, `appendRowSA`, etc. in `src/lib/sheets.js`) to Postgres-backed equivalents. directory's 28 Sheets operations + 2 Drive operations follow automatically. The Drive proxy specifically may stay Drive-backed (Drive isn't migrating to Postgres) or move behind a CDN if the gmapImg files get hosted elsewhere - separate decision.

### Dashboard SA migration (Bundle 3 PR C) [PROD BUG FIXED]

**This entry documents two things equally: the migration AND a 2-month production data-loss bug surfaced and fixed during the recon.**

#### The migration

- **What:** `src/app/api/dashboard/route.js` fully migrated user-OAuth to canonical SA. The last general-purpose user-OAuth Sheets WRITE surface in the codebase (only intentional `invoiceActions.js` user-flow paths remain - those are NOT a consolidation target). 12 user-OAuth call sites migrated: 2 reads (`readSheet` → `readSheetSA`), 2 appends (`appendRow` → `appendRowSA`, both width=6 A:F for `news_interactions`), 8 cell updates (`updateCell` → `updateCellSA`, the bug-correcting sub-phase, see below). Import collapsed from 7 names (`readSheet, readSheetSA, appendRow, appendRowSA, updateCell, updateCellSA, SHEET_IDS`) to 4 (`readSheetSA, appendRowSA, updateCellSA, SHEET_IDS`). Token extraction (`const token = session.accessToken;`) removed from both GET and POST handlers. `if (!token)` check removed from POST. The `if (!session)` auth check stays in both handlers (still require authentication). File 401 -> 394 LOC (-7).
- **Why:** Completes the user-OAuth Sheets WRITE consolidation across the codebase. After PR #55 (people Sheets), PR #56 (people Gmail), PR #58 (directory Sheets + Drive), and this PR C, the only user-OAuth Sheets/Drive surface remaining is `invoiceActions.js`'s user-flow invoice submission paths - deliberately user-OAuth so the invoice email and Drive upload appear "from" the submitter.
- **The cleanest Scenario B:** User identity (`email`) was ALREADY captured as a data column (`news_interactions` col B = userEmail) from `session.user.email` at handler entry, NEVER derived from the OAuth token. Both append paths (L352 batch, L393 single-post) explicitly embed `email` as the second element of the row. The 8 cell-update paths target rows whose identity is already persisted in col B (identified via postId+email match in the read). Result: the SA swap preserved identity with **zero schema touch** - the actor changed (user → SA), the recorded identity (the email in the row) did not.
- **Where:** `src/app/api/dashboard/route.js` (the only code file touched in PR C).
- **LOC:** -7 in `dashboard/route.js`. Small diff because most of the file (the GET bootstrap) was already SA-canonical (`readSheetSA` × 4 from earlier work); PR C just migrated the POST handler's news_interactions ops.

#### The bug (load-bearing find from recon)

- **What:** The 8 `updateCell` calls in dashboard/route.js had a pre-existing **WRONG-ARG-SHAPE**: they passed 5 positional args to a 4-arg helper. The canonical `updateCell` in `src/lib/sheets.js` is `updateCell(accessToken, spreadsheetId, range, value)`. The dashboard calls were `updateCell(token, COLLECTION, "news_interactions", \`C${sheetRow}\`, "TRUE")`. The function silently mapped the args positionally: `accessToken=token`, `spreadsheetId=COLLECTION`, `range="news_interactions"` (the TAB NAME, not the cell address), `value="C${sheetRow}"` (the intended range string), and the real value `"TRUE"`/`now` was silently **DROPPED** as the unbound 5th arg.
- **Effect:** Every cell update silently wrote the intended range string ("C5", "D5", "E5", "F5" etc.) to **cell A1 of news_interactions** instead of writing the value to the target cell. A bare tab name in the Sheets `values.update` API resolves to A1, and `values: [["C5"]]` writes a 1×1 grid starting there. Each broken call overwrote A1 with the most-recent intended range string.
- **Empirical confirmation (recon SP2):** A read of `news_interactions!A1` returned `"E16"` - a stray range string where the header `"postId"` should be. Other header cells (B-F: `userEmail`, `read`, `readAt`, `saved`, `acknowledged`) were intact - the bug only ever wrote to A1. Timestamps in the data rows from 2026-03-21 onward → **~2 months in production**.
- **User-facing impact:** The FIRST interaction on a post persisted (append path was shape-correct → row gets created with read=TRUE, readAt=now, the user's interaction recorded). Every SUBSEQUENT interaction on the same post silently did NOT persist (the update path → A1 corruption, target cell unchanged). Toggle "saved", toggle "acknowledged", mark-all-read on an already-touched post - all silently reverted on next page refresh.
- **Why it stayed invisible:** Three things stacked: (a) errors swallowed (`catch (error) { ... return { success: false } }` - no throw), (b) callers used `await Promise.all(updates)` without inspecting the returned `{success, error}` objects (so silent 400s would have been invisible anyway), (c) low-frequency feature where users don't usually notice "I saved this post and it forgot." Plus reads use `r[0]` indexing not header-name lookup, so the corrupted A1 header never broke read logic.
- **The FIX:** The SA migration corrected the arg shape as a direct consequence of using `updateCellSA(spreadsheetId, range, value)` correctly - the range arg is now the full tab-prefixed string `news_interactions!C${sheetRow}`, the value lands in arg 3, the API receives the right call shape and the value reaches the cell. **Frame: "canonicalization corrected a pre-existing wrong-arg-shape," NOT "accidentally fixed a bug."** Ad-hoc call sites with mismatched signatures are exactly what canonicalization eliminates - this bug is the strongest single argument for the whole consolidation effort.
- **MANUAL POST-MERGE STEP (one-time data correction):** A1 still holds `"E16"` after the code migration. The new code writes to C/D/E/F only, never to A1, so A1 will NOT self-heal. Kevin restores A1 to `"postId"` manually in the Sheets UI post-merge - 5-second click. **Deliberately NOT a code-driven write:** the migration code shouldn't carry a one-time cleanup that would never fire again, and the corruption is a data-layer artifact not a code concern. Documented here as the canonical record.
- **POST-MERGE SMOKE TEST (broken-paths-now-fixed):**
  - Mark a post as read for the FIRST time → persists on refresh (was working - sanity check)
  - On the SAME post, toggle "saved" → persists on refresh (**was BROKEN for ~2 months, now fixed - load-bearing check**)
  - On the SAME post, toggle "acknowledged" → persists on refresh (**was BROKEN, now fixed**)
  - `mark-all-read` on a mix of new posts + already-touched posts → all persist (was partial: new ones worked via append, already-touched ones silently lost)
