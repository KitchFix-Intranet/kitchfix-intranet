...
- P1 trust + polish bundle (items 4 / 5 / 1 / 6) - one completeness rule (`entered || no-service`) unified across MonthCard + PeriodCard + FullSeasonCard + yearBannerStats + export BY MONTH sheet + workspace (which was the correct reference); top-nav Service Calendar re-runs F2 landing on the clean-URL transition; extras band restyled per render R1a-navy (navy left accent matching sc-day-group-header + white ground + hairlines); workspace ghost tiles per render R4b (in-period no-metadata dates, dashed border, inert). Full brief: `docs/… SC_7UPDATES_PLAN_v2`.
- F6 phase foundation (2026-07-10) - `sc_phase_calendar(account_key, phase, start_date, end_date)` seeded with 48 rows across 5 PDC accounts (CIN-AZ 13 / TXR-AZ 10 / TBR-FL 9 / TBJ-FL 8 / STL-FL 8; TBJ + STL Kevin-approved as TBR-peer arcs 2026-07-10); operator export gains a year-scope BY PHASE table (`Phase · Dates · Proj $ · Actual $ · Meals`) placed between SERVICE PERFORMANCE and BY PERIOD, dollars sourced from the same per-day view rows the Summary aggregates from (no second money path), attribution via date-range join. Phase names stored as-recorded (canonical vocab deferred to Stage 2 per SC_PDC_PHASES.md); non-PDC accounts + unseeded PDC accounts + pre-migration state all degrade cleanly to a byte-identical pre-F6 workbook.
- P2 notes UX (2026-07-10, items 2 + 3) - the DayDetail note composer now leads as the ride-along "Note for this save" field above the action bar; the draft joins `sc-submit-day` as `rideNote` and appends via the authored-note path AFTER save success (server-derived author). Post-save note-append failure surfaces as `noteFailed:true` and drives an honest partial toast ("Saved - note couldn't post, use Add note") - never a clean success when the note didn't land. Review overlay's retired Latest note history echo replaced by NOTE RIDING THIS SAVE block (sc-tint ground, quoted draft, Edit + Remove) - Edit pops back to entry with the ride-along focused, Remove clears the draft in-place. Subordinated "+ Add a note without saving" link below the action bar exposes the standalone `sc-add-note` wire (collapsed by default; same shared draft). Queue replay persists `rideNote` on the queued payload so a queued close on a network-failed save clean-closes rather than routing the draft through discard-confirm. Bulk endpoints do NOT accept `rideNote` (v1 fence - a single note across N days blurs which day it was meant for). DaySquare gains the 11px chat-bubble corner glyph at ~50% opacity when `day.noteEntries.length > 0` (Q-b: NOTE entries only, history rows never count) on both compact and full tile variants; sm-tile signal fed by a new year-summary `hasNoteEntries` boolean (Set-based dates lookup via `readNoteDatesInRange`), lg drill-in reads existing `day.noteEntries`. Legend popup picks up a "Has notes" row in Calendar context. BY PHASE export rider (owner ruling b): meals column split into `Proj meals · Actual meals` pair so partial phases read honestly by construction; both columns foot to the year TOTAL MEALS headlines.
