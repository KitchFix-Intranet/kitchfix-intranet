...
- `docs/design-tokens.html` - CURRENT token inventory (every token rendered; includes the four `--accent-sc*` variants + tool accents).
- `docs/design-tokens-v3.html` - design-system narrative from the token audit (3-tier architecture, WCAG contrast report, icon concept -> glyph vocabulary, interactive states, composite screen demo). v3 predates the SC accent family - use both together: v3 for the system's WHY, `design-tokens.html` for the current WHAT.
- `docs/SC_DESIGN_TOKEN_README.md` - THE SC-specific token bible for the drill-in redesign. Read this before proposing anything: it carries the operational alignment rules ("every green -> --accent-sc*", "every state -> --status-*", etc.), per-surface consumption reference, a pre-flight checklist, and the Claude-vs-Kevin decision split.
