...
>
> **Account**: TBR - FL (Tampa Bay Rays — MLB @ Tropicana Field + MiLB Charlotte County PDC). **Shape**: MLB per-meal (no SF); MiLB **SF% 25%** ($382,448 one-time 2024) discounts per-meal. Level: PDC (`billing_model=actuals_drive_invoice`).
...
- **MONEY_MODEL digest row**: `TBR - FL | SF% 25% on MiLB only | $27.95 MiLB / $39.48 MLB | $20.96 MiLB / $39.48 MLB | $382,448 one-time 2024 | Yes, meal revenue`.
- **PG**: `billing_model = actuals_drive_invoice`, `has_homestand_schedule = false`.

## §2. Contract evidence (verbatim)

### 2.1 MLB per-meal 2024 rates (Major League SOW § 6(i)(a)-(b) p.4)
...
### 2.2 MiLB per-meal 2024 rates (Minor League SOW § 6(a)(i-iv) p.5)
...
### 2.3 25% MiLB discount + $382,448 SF (Minor League SOW § 6(c) p.6) — **VERBATIM**

> "(c) Service Fee.
> In addition to the compensation described above, the Club shall pay to the Provider a service fee (the 'Service Fee') in the amount of three hundred eighty-two thousand four hundred forty-eight dollars (USD $382,448.00), plus applicable taxes. The Rates will be reduced by 25% for all billings for the Minor League Baseball Teams within the Term. The Club will pay the Service Fee in accordance with the following schedule:
...
**$382,448 SF is paid as two installments both completed by Feb 1, 2024.**

### 2.4 CPI escalation — MLB (Major League SOW § 6(i)(c) p.5)
...
### 2.5 CPI escalation — MiLB (Minor League SOW § 6(a)(v) p.5)
...
### 2.7 C-2 chase — SF renewal for 2026
...
**Verdict: UNKNOWN — CANNOT DETERMINE FROM CONTRACT** whether the $382,448 recurs in 2025+ or is one-time.

### 2.8 Invoice cadence + Net terms (MLB SOW § 6(ii) p.5; MiLB SOW § 6(b) pp.5-6)

Both SOWs (identical language):
> "Billings and Reconciliations. Within five (5) days following the final day of each Calendar Week (or partial Calendar Week, as applicable) that falls during the SOW Term, the Provider will deliver to the Club an invoice for the amount payable by the Club in connection with the Meals prepared and served by Provider pursuant to this Agreement during the applicable Calendar Week (or partial Calendar Week, as applicable). ... invoices hereunder will include clear information and supporting documentation specifying on the specific Service Days, Meals, as well as any necessary Club confirmations or approvals ..."

**Weekly invoicing.**

### 2.9 Dispute mechanism (MLB SOW § 6(iii) pp.5-6; MiLB SOW § 6(d) p.6)
...
### 2.10 Force Majeure suspension (Services Agreement Major League § 4 p.4; MiLB § 4 p.4)
...
CPI-linked adjustment on per-meal (§ 6(i)(c) MLB / § 6(a)(v) MiLB) is the only rate-adjustment mechanism; not on the $382,448 SF. **UNKNOWN** on SF true-up.
...
### 3.1 Invoice K300168545 — 2026-03-01 TBR-FL MLB (Spring Training @ Charlotte Sports Park, invoiced to Tropicana Field HQ)
...
- **Line items**:
  ...
  - `TBR MiLB - Lunch/Dinner` @ **$21.68** (qty 92-240/day, 7 days: 6/29, 6/30, 7/1, 7/2, 7/3, 7/4)
  - `TBR MiLB - Road Sandwiches` @ **$15.00** flat, qty 28 (6/30, 7/2)
  - `TBR MiLB - Breakfast` @ $17.83 (varies)
  - `Labor Fee` — Labor Fee $280.00 (07/03)
  - `Extra Protein (TBR) - Chicken/Pork` $111.84 (07/03)

**Rate discrepancy vs MONEY_MODEL digest**:
- MiLB Lunch/Dinner **$21.68 INVOICE** vs **$20.96 DIGEST** — **CONFLICT** ($0.72/meal delta).
...
`sc_service_prices` for TBR-FL:

| service | is_flat_fee | is_tax_free | price_kind | price |
|---|---|---|---|---|
| MLB Breakfast | | | projected | ? |
| MLB Lunch/Dinner | | | projected | 39.48 |
| MiLB Breakfast | | | projected | ? |
| MiLB Lunch/Dinner | | | projected | 20.96 |

(Exact PG values in the PG appendix — MONEY_MODEL digest shows $20.96 MiLB and $39.48 MLB.)
...
- **C-2**: Whether the $382,448 SF recurs in 2025 or 2026: **contract SILENT**. Invoice sample shows NO SF line. Best evidence: **one-time 2024 payment, not recurring**.
- MiLB rate reconciliation: MONEY_MODEL $20.96 vs invoice $21.68 — needs Kevin's ruling.
...
- B&G (Boys & Girls Club) $6.50/lunch (MONEY_MODEL §Open items): not in any of the four TBR-FL contract files. Confirms open-items list.
...
- **§C-2 CONFIRMED**: contract silent on 2026 SF recurrence. Invoice sample shows no SF line. Best evidence: one-time 2024 payment, not recurring. **This is the C-2 conflict Kevin's brief flagged. Resolved to "SF was one-time 2024" absent further contract paperwork.**
- **§MiLB rate mismatch**: invoice $21.68 vs MONEY_MODEL $20.96 — $0.72/meal delta. Needs Kevin's ruling on which is correct.
...
- **§B&G**: MONEY_MODEL §Open items lists $6.50/lunch for Boys & Girls Club; not in TBR contracts on file.
