...
| `SLACK_SC_BILLING_WEBHOOK_URL` | Incoming-webhook URL for **`#service-calendar-invoices`**, the SC-billing notifications channel (N2 push-failed + future N3.3 past-due). Added PR-F 2026-08-13 per addendum §A6b. **Distinct from `SLACK_SC_WEBHOOK_URL`** (operational SC alerts). Read at `src/lib/billing/qboNotifications.js` `sendSlack()`. Missing = Slack post silently skipped; email delivery is unaffected. Test-mode posts to this channel ALWAYS open with a `[TEST]` prefix + close with `TEST_SLACK_FOOTER` so a member who joins later cannot mistake one for real. | Prod, Preview | Slack notifications silently skipped; email still sends |
...
| Variable | Description | Scope | If missing |
|---|---|---|---|
| `QBO_PROXY_BASE` | Prefix of Josh's QBO proxy: origin + `/qbo` (currently `https://chief.ngrok.app/qbo`). The `/v3/company/{realm}` segment is composed at call time from `QBO_REALM_ID` below - the split lets Josh rotate proxy hosting without touching realm and vice versa. The proxy holds the Intuit OAuth tokens server-side. Read by `scripts/billing/diff-week.mjs` (GET) and by `src/lib/billing/qboAdapter.js` (POST). | Local `.env.local` (dev + probe scripts); Prod when PR-C activates | Diff harness + adapter fail before any network call |
| `QBO_REALM_ID` | Numeric QBO realm (company) id, currently `1219933770`. Composed into URLs by `composeInvoiceUrl` in `src/lib/billing/qboAdapter.js` and by the diff harness. | Local `.env.local`; Prod when PR-C activates | Adapter throws `postInvoiceDraft: QBO_REALM_ID required` before any network call |
