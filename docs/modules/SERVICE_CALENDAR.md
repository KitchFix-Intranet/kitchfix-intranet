...
**Proposed hardening (TODO, awaiting Kevin's go)**: a workflow that scans PR head for new `docs/migrations/*.sql` and requires an explicit "sc-XX applied in Studio: YES" review-comment before the merge button unlocks. Not yet built. Documented in [`SC_STATUS.md`](../SC_STATUS.md) "Remaining work".
