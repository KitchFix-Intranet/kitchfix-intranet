...
- **Month + Period - AAA per-meal + PDC overlay + PDC per-meal** (spec Sheet 6/7, corrective wave 2026-07-13):
  - **AAA drill (CIN - KY, TBJ - NY): LIVE**. Approved product; not part of the redesign.
  - **PDC + PDCO drill (CIN - AZ, TXR - AZ, TBR - FL, STL - FL, TBJ - FL): PARKED behind Coming Soon 2026-07-13** pending the wall-poster redesign. The current sheet (described below) is superseded-in-waiting - kept in the codebase so proofs regenerate internally for the design side, but the ExportControl menu greys the drill items with a `COMING SOON` tag + `/api/service-calendar/print` returns 404 for `scope=month|period` on PDC/PDCO account keys (defense-in-depth). Redesign arc + resume procedure in [`../design/PDC_PRINT_REDESIGN.md`](../design/PDC_PRINT_REDESIGN.md). Season PDF, Ops Calendar PDF, and Excel exports for these accounts stay live.
  - **Grammar (as of the current sheet, for the design side's reference):** full state-fill grid + game overlay + meal stack. Applies uniformly across AAA (live), PDCO (parked), PDC (parked) per R4. AWAY days on AAA render `--awayfill` + grey `@OPP` (`.awy`) like the MLB sheet grammar; the meal stack overlays if the day carries service counts. GAME days on AAA / PDCO render `--homefill` + navy `.opp` + `.tm` time + meal stack. **R6**: past game days without actuals render `.nd` (NO ACTUALS copper) + opp + time + **no meal stack** (projections don't print on past days). Spring copper title chip fires on PDC / PDCO variants when the scope intersects a spring block. **Period pills at boundaries** (see below).
...
**Two chromium sources by design - `npm run dev` cannot generate PDFs.** The API route above uses `puppeteer-core` + `@sparticuz/chromium`; that package ships a Linux x86_64 chromium binary because the whole point of it is to satisfy the Vercel / AWS Lambda serverless runtime with a chromium small enough to bundle. On macOS Node cannot execute a Linux ELF and the route fails with `spawn ENOEXEC` at `puppeteer.launch()`. This is not a regression - production is fine, verified 2026-07-31 by hitting `/api/service-calendar/print?scope=season` on `kitchfix-intranet.vercel.app` for TBJ - FL and CIN - OH (both `200 application/pdf`). Excel exports from the same menu are unaffected; they build in-process with no subprocess spawn. **For local proofs use `scripts/sc-print/gen-all-pdfs.mjs`**, which imports the full `puppeteer` package (a separate dependency, distinct from `puppeteer-core`) that downloads and bundles a native chromium at install time. **For user-facing verification use preview or production.** If the "two chromium packages in `package.json`" arrangement ever looks like a mistake worth cleaning up, this is what it is for.
