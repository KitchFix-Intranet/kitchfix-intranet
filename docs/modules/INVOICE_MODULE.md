...
- **`hasDigitalPDF` guard silently suppressed line-item extraction for photo uploads**. A guard added 2026-04-03 (commit `223edbd`) at `triggerAIScan` to dodge unreliable phone-photo extraction. When STL-MO's submitter switched to phone uploads ~2026-04-15, 133 submissions silently got zero line items for 7 weeks before anyone noticed. Removed in PR #108. The defect was the absence of reconciliation, not the guard itself: nothing checked "submissions that should have line items but don't." Recovered via `scripts/backfill-stl-mo-line-items.mjs`.
- **Kuna Foodservice photo extraction is unreliable**. Multi-page, dense, often photographed sideways. Photo OCR undercounts (missed pages) or overcounts (hallucinated lines on dense images); not fixable by model tuning, requires source PDFs. 6 invoices held in `review_queue` with `reason=overcount_suspect_reextract` pending PDF re-source: 215530-00, 227715-00, 229838-00, 236964-00, 237201, 244833-00.
