...
5. `[ran] scripts/sousai-agent-test.mjs` - first attempt 6/7 (case 1a run 1 miss), second attempt clean 7/7 both runs. **Correction to the original characterization (2026-07-31 - the account-shape-awareness PR carries this amendment): this was NOT a flake.** Case 1a exists specifically to catch the enumeration-shortcut behavior - the prompt forbids answering an enumeration question ("which accounts are flat-fee") from `search_documents` snippets, and case 1a's grader fires when the model takes that shortcut. On the first attempt, run 1 answered from snippets and happened to land the correct five accounts; the grader correctly fired FAIL. Re-running to green hid a real datum. The measured behavior is: **the model took the snippet shortcut on 1 of 4 runs of this question** in the post-revoke session. Neither the revoke nor the read path caused it - identical service-role reads landed on both attempts. Record as a stochastic behavior measurement for the eventual eval set's flake tracking, not as a flake to move past.
