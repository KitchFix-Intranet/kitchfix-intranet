...
**Last updated:** 2026-05-27 (post-PR-#69 merge; combined state-of-truth update covering yesterday's cutover + today's PR A). **news_interactions CUTOVER COMPLETE 2026-05-27 (module 1 fully migrated to Postgres).** Reads serve from Postgres (`READ_FROM_POSTGRES=news_interactions` live in Vercel); dual-write remains ON as the rollback net (`DUAL_WRITE_TABLES=news_interactions`, drop in a later session once stability is confirmed). Backfill ran clean: 18 rows loaded, ON CONFLICT upsert reconciled NP-009 (already in PG from dual-write), post-write count verified 18 matching Sheets exactly. All interaction types (mark-read, save, ack, mark-all-read) verified persisting through PG read path end-to-end. **The dual-write pattern is now PROVEN LIVE.** **Module 2 (directory) PR A SHIPPED 2026-05-27 as PR #69** - dormant scaffolding: dataStore adapters for accounts + hero_images, 3 shared primitives (coordinatedWrite, deleteRecord, replaceScope) as foundation for PR B. Built from `docs/SHEETS_AUDIT*.md` LIVE verdicts: dead cols N/O/P/Q dropped, dir_links folded into accounts on PG, col-T region excluded from writeable set (latent-blanking fix). hero_images dormant per-account infra (team_key NULL today). Write mechanism uses `batchUpdateRangesSA` (signature verified against production callers; rate-limit safe). Append path A-R parity with current handler. Directory handler NOT rewired - flags off = byte-identical, zero-merge-risk dormant infrastructure (same pattern as Stage 1 PR 1). **Next: directory PR B** - the activation PR (handler rewire + contacts + work_locations adapters + col-G fix + per-module read flags). PR #68 (orphaned interim dashboard update) was closed; its content shipped via PR #69's combined diff and is reflected here. Mode = fast-as-safe (`docs/MIGRATION_APPROACH.md`).
**Stage:** 1 (Supabase Setup + Schema Design) - kickoff; Stage 0 ~99%, only Calendar consolidation deferred
**Next milestone:** directory PR B (handler rewire + contacts + work_locations adapters + col-G fix + per-module read flags - the module 2 activation PR)
...
- **PRs shipped:** 34 (since 2026-05-14)
- **Stage 0 progress:** ~99% (only Calendar SA helper consolidation remains, deferred post-Stage-1)
- **Stage 1 progress:** in flight - **Module 1 (news_interactions) CUT OVER 2026-05-27** - reads serve from Postgres, dual-write on as rollback net, 18 rows matching Sheets <-> PG, all interaction types verified persisting through PG path, dual-write pattern proven LIVE end-to-end. **Module 2 (directory) PR A MERGED 2026-05-27 as PR #69** - dormant scaffolding: dataStore adapters for accounts + hero_images, 3 shared primitives (coordinatedWrite, deleteRecord, replaceScope) for PR B, batchUpdateRangesSA write mechanism. **Next: directory PR B** (handler rewire + contacts + work_locations adapters + col-G fix + per-module read flags). **Sheets audit complete** (`docs/SHEETS_AUDIT*.md`); BUILD RULE = migrate from LIVE verdicts, not sheet structure. user_accounts auth mappings drafted. Mode = fast-as-safe (`docs/MIGRATION_APPROACH.md`).
...
11. **Stage 1 IN FLIGHT** - Supabase project live, schema design via the `docs/SHEETS_AUDIT*.md` three-part reference, phased module-by-module cutover. **Module 1 (news_interactions) CUT OVER 2026-05-27** - dual-write scaffolding + dataStore shipped as PR #61, backfill script shipped as PR #63. Cutover executed: `DUAL_WRITE_TABLES=news_interactions` flipped in Vercel -> `scripts/backfill-news-interactions.mjs --execute` ran clean (18 rows loaded, ON CONFLICT upsert reconciled NP-009 from dual-write, count verified 18 in PG = 18 in Sheets) -> `READ_FROM_POSTGRES=news_interactions` flipped -> all interaction types (mark-read, save, ack, mark-all-read) verified persisting through PG read path. **Reads now serve from Postgres; dual-write stays on as the rollback net** (drop from DUAL_WRITE_TABLES later once stability is confirmed). **The dual-write pattern is PROVEN LIVE.** **Module 2 (directory) PR A MERGED 2026-05-27 as PR #69** - dormant dataStore adapters for accounts + hero_images + 3 shared primitives (coordinatedWrite, deleteRecord, replaceScope), built from audit LIVE verdicts (dead cols N/O/P/Q dropped, dir_links folded into accounts on PG, col-T region excluded from writeable set, hero_images team_key NULLABLE for future per-account direction). batchUpdateRangesSA write mechanism signature verified against production callers. Handler NOT rewired - same zero-merge-risk dormant pattern as Stage 1 PR 1. **NEXT: directory PR B** - the activation PR (handler rewire + contacts adapter with col-G fix + work_locations adapter + per-module read flags + multi-table coordinated write enabled in admin-update-account). **Migration order (from synthesis, applies the proven pattern):** news_interactions (CUT OVER 2026-05-27) -> directory PR A SHIPPED + PR B next -> People Portal submissions + incidents + drafts/notification_log -> Invoice config -> Smart Inventory (with AI cron as external writer) -> invoice_submissions_26 finance-critical -> defer service-calendar/performance/GL_CODES. **BUILD RULE** (PR #65): Supabase schema follows audit LIVE verdicts, NOT sheet structure - dead/orphan/empty/reference data does NOT migrate. **Real migration scope: ~25 LIVE tables of substance, not 82.** user_accounts auth mappings drafted (4 admins + Regionals: Shane=East / Ryan=West / Dec=both + 21 site staff); account-key format MEASURED canonical = spaces ("CIN - OH"); hyphen quarantined in deferred modules. Mode = fast-as-safe (`docs/MIGRATION_APPROACH.md`).
