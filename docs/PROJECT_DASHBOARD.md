...
**Last updated:** 2026-05-22 (PR C complete in working tree, pre-merge; CLAUDE.md item 1 CLOSED, directory consolidation COMPLETE, dashboard SA migration COMPLETE)
...
- **PRs shipped:** 24 (since 2026-05-14)
- **Stage 0 progress:** ~96%
- **Items remaining:** 9 (in 5 bundles + 6 unbundled; CLAUDE.md item 1 closed PR #56; latent directory POST auth gap closed PR #57; directory SA migration complete PR #58)
- **Calendar estimate to Stage 1:** 2-3 months at sustainable pace

---

## Done - 24 PRs shipped to main
...
7. ~~Bundle 3 sub-phase 0 (recon)~~ - ✅ SHIPPED 2026-05-19 as PR #53. ~~Bundle 3 PR A1 (cron consolidation, 3 files)~~ - ✅ SHIPPED 2026-05-20 as PR #54. ~~Bundle 3 PR A2a (people/route.js Sheets consolidation + Drive consolidation + ops dead-imports)~~ - ✅ SHIPPED 2026-05-22 as PR #55. ~~Bundle 3 PR A2b (Gmail SA canonicalization + crypto.subtle JWT block removal)~~ - ✅ SHIPPED 2026-05-22 as PR #56. **The Bundle 3 hand-rolled-JWT-consolidation sub-bundle (PR A1 + A2a + A2b) is COMPLETE - CLAUDE.md item 1 CLOSED.** PR B SPLIT into B1 (security fix) + B2 (the SA migration). ~~Bundle 3 PR B1 (directory POST interim admin gate)~~ - ✅ SHIPPED 2026-05-22 as PR #57. ~~Bundle 3 PR B2 (directory/route.js full SA migration - 13 reads + 13 writes + 2 Drive calls + client cleanup, -114 LOC)~~ - ✅ SHIPPED 2026-05-22 as PR #58. **The Bundle 3 directory sub-bundle (B1 + B2) is COMPLETE - directory is 100% SA + has the interim admin gate.** ~~Bundle 3 PR C (dashboard/route.js SA migration + wrong-arg-shape bug fix)~~ - READY FOR MERGE 2026-05-22 (working tree, awaiting commit + PR). The last general-purpose user-OAuth Sheets write surface is closed. Manual post-merge step: Kevin restores news_interactions!A1 to "postId" in the Sheets UI. **Remaining work:** Calendar SA helper consolidation (post-Bundle-3 follow-up, deferred) + **Stage 1 auth strategy decision (the LAST Stage 0 gate)** - where the directory B1 interim env-var gate gets replaced by the Supabase role model + admin dashboard, which also resolves the admins-tab flag-model divergence between people.js (col A + col C "hr") and the rest of the codebase.
