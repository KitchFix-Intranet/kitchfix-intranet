...
**Last updated:** 2026-05-26 evening (post-PR-#65 merge; 5 PRs shipped today: #61 Stage 1 PR 1 dual-write scaffolding + dataStore, #62 npm audit fix, #63 backfill script dry-run-verified, #64 dashboard sync, #65 Sheets audit 3-part reference. **news_interactions cutover is the NEXT action (Wednesday, Kevin manual in Vercel):** sequence is set DUAL_WRITE_TABLES=news_interactions -> run scripts/backfill-news-interactions.mjs --execute -> set READ_FROM_POSTGRES=news_interactions. Flags currently empty (Sheets-only, dormant). **Directory = migration module 2, FULLY DESIGNED (not built):** schema (4 tables: accounts with dir_links folded in, contacts A-G only, work_locations kept as real table, hero_images), 3 new dataStore primitives (delete, replace-list, multi-table coordinated write), per-module read flags, Sheets-first/PG-best-effort atomicity. Build AFTER cutover proves the pattern. Gotcha flagged: accounts col-T (Region) write-boundary must be preserved (latent blanking risk, same class as the col-G bug). **Sheets audit COMPLETE (docs/SHEETS_AUDIT*.md, 2,239 LOC):** real migration scope is ~25 tables of 82 total (19 empty, 6 schema-only, ~11 orphans); the rest is cleanup or defer. **BUILD RULE established:** Supabase schema is built from audit LIVE verdicts, NOT sheet structure - dead/orphan/empty/reference data does NOT migrate. user_accounts auth mappings drafted (4 admins + Regionals + 21 site staff, weeks out). Account-key format MEASURED = spaces canonical ("CIN - OH"); hyphen inconsistency quarantined in deferred modules.
...
- **Stage 1 progress:** in flight - PR 1 shipped (dual-write scaffolding + news_interactions dataStore, dormant until cutover); cutover flags pending Wednesday flip. **Module 2 (directory) FULLY DESIGNED** (not built); builds after cutover. **Sheets audit complete** (docs/SHEETS_AUDIT*.md); BUILD RULE = migrate from LIVE verdicts, not sheet structure. user_accounts auth mappings drafted.
...
11. **Stage 1 IN FLIGHT** - Supabase project live, schema design via the new docs/SHEETS_AUDIT*.md three-part reference, phased module-by-module cutover. **Module 1 news_interactions** SHIPPED as PR #61 (dual-write scaffolding + dataStore, dormant pending flag flip); backfill script SHIPPED as PR #63 (dry-run-verified, NOT run live yet). **Cutover sequence (Wednesday, Kevin manual in Vercel):** set DUAL_WRITE_TABLES=news_interactions -> run `node --env-file=.env.local scripts/backfill-news-interactions.mjs --execute` -> set READ_FROM_POSTGRES=news_interactions -> watch divergence -> later drop from DUAL_WRITE_TABLES. **Module 2 directory** FULLY DESIGNED 2026-05-26 (not built); 4-table schema with dir_links folded into accounts, contacts A-G with col-G bug fix during build, work_locations kept real, hero_images; needs 3 new dataStore primitives (delete, replace-list, multi-table coordinated write); per-module read flags; accounts col-T (Region) write-boundary preservation flagged as latent risk. Build AFTER cutover proves the pattern. **Migration order (from synthesis):** news_interactions (done, cutover tomorrow) -> directory -> People Portal submissions + incidents + drafts/notification_log -> Invoice config -> Smart Inventory (with AI cron as external writer) -> invoice_submissions_26 finance-critical -> defer service-calendar/performance/GL_CODES. **BUILD RULE** (PR #65): Supabase schema follows audit LIVE verdicts, NOT sheet structure - dead/orphan/empty/reference data does NOT migrate. **Real migration scope: ~25 LIVE tables of substance, not 82.** user_accounts auth mappings drafted (4 admins + Regionals: Shane=East / Ryan=West / Dec=both + 21 site staff); account-key format MEASURED canonical = spaces ("CIN - OH"); hyphen quarantined in deferred modules.
