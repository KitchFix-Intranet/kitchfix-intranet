...
**Last updated:** 2026-05-22 (PR B1 complete in working tree, pre-merge; CLAUDE.md item 1 CLOSED)
...
- **PRs shipped:** 22 (since 2026-05-14)
- **Stage 0 progress:** ~94%
- **Items remaining:** 10 (in 5 bundles + 6 unbundled; CLAUDE.md item 1 closed PR #56)
- **Calendar estimate to Stage 1:** 2-3 months at sustainable pace

---

## Done - 22 PRs shipped to main
...
7. ~~Bundle 3 sub-phase 0 (recon)~~ - ✅ SHIPPED 2026-05-19 as PR #53. ~~Bundle 3 PR A1 (cron consolidation, 3 files)~~ - ✅ SHIPPED 2026-05-20 as PR #54. ~~Bundle 3 PR A2a (people/route.js Sheets consolidation + Drive consolidation + ops dead-imports)~~ - ✅ SHIPPED 2026-05-22 as PR #55. ~~Bundle 3 PR A2b (Gmail SA canonicalization + crypto.subtle JWT block removal)~~ - ✅ SHIPPED 2026-05-22 as PR #56. **The Bundle 3 hand-rolled-JWT-consolidation sub-bundle (PR A1 + A2a + A2b) is COMPLETE - CLAUDE.md item 1 CLOSED.** PR B has been SPLIT into B1 (security fix) + B2 (the SA migration). **Bundle 3 PR B1 (directory POST admin gate, interim env-var-based, closes latent authorization gap) - READY FOR MERGE 2026-05-22 (working tree, awaiting commit + PR). Required deploy step: DIRECTORY_ADMIN_EMAILS env var must be set in Vercel before merge.** Next: PR B2 (directory/route.js full SA migration - 13 direct API calls + Sheets/Drive client consolidation, the original PR B refactor scope). After B2: PR C (dashboard/route.js, Scenario B user-OAuth → SA migration for the 10 news_interactions writes). Calendar SA helper consolidation deferred to post-Bundle-3. Stage 1 auth strategy decision (the last Stage 0 gate) is where directory's interim env-var gate gets replaced by a Supabase-backed role model.
