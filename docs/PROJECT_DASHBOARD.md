...
**Last updated:** 2026-05-27 (**news_interactions CUTOVER COMPLETE - module 1 fully migrated to Postgres**). Reads now serve from Postgres (`READ_FROM_POSTGRES=news_interactions` live in Vercel); dual-write remains ON as the rollback net (`DUAL_WRITE_TABLES=news_interactions`, will drop in a later session once stability is confirmed). Backfill ran clean: 18 rows loaded via `node --env-file=.env.local scripts/backfill-news-interactions.mjs --execute`, ON CONFLICT upsert reconciled NP-009 (already in PG from dual-write), post-write count = 18 matching Sheets exactly. All interaction types (mark-read, save, ack, mark-all-read) verified persisting through the PG read path end-to-end. **The dual-write pattern is now PROVEN LIVE.** Stage 1 PR 1 infrastructure (cutover.js / dataStore.js / supabase.js / dashboard handler unchanged since #61) is no longer dormant - it is the production data path for news. **Next: module 2 (directory) build** - fully designed 2026-05-26, ready to apply the proven pattern (~3-4 PRs estimated). Mode = fast-as-safe (`docs/MIGRATION_APPROACH.md`). Cutover code/flags untouched in this commit - this is a docs-only state update.
**Stage:** 1 (Supabase Setup + Schema Design) - kickoff; Stage 0 ~99%, only Calendar consolidation deferred
**Next milestone:** directory build (module 2, fully designed 2026-05-26, ~3-4 PRs - applying the now-proven dual-write pattern)
...
- **PRs shipped:** 31 (since 2026-05-14)
- **Stage 0 progress:** ~99% (only Calendar SA helper consolidation remains, deferred post-Stage-1)
- **Stage 1 progress:** in flight - **Module 1 (news_interactions) CUT OVER 2026-05-27** - reads serve from Postgres, dual-write on as rollback net, 18 rows matching Sheets <-> PG, all interaction types verified persisting through PG path, dual-write pattern proven LIVE end-to-end. **Module 2 (directory) FULLY DESIGNED 2026-05-26** - next to build (~3-4 PRs). **Sheets audit complete** (`docs/SHEETS_AUDIT*.md`); BUILD RULE = migrate from LIVE verdicts, not sheet structure. user_accounts auth mappings drafted. Mode = fast-as-safe (`docs/MIGRATION_APPROACH.md`).
