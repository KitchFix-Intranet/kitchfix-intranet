...
- `/api/cron/backup-sheets` - 06:00 UTC daily
- `/api/cron/schedule-drift` - 11:00 UTC daily (06:00 ET, Stage 1 watchdog)
...
- **2026-07-13 (#428 amendment)** - Period pills + AAA season squish fix. Every day cell that is a fiscal period boundary renders a pill at top-right: navy fill = `Pn STARTS`, navy outline = `Pn ENDS`. Applies to every drill variant (MLB, AAA, PDC, PDCO) at both month and period scope. Supersedes the v2 spec's `Period-boundary line + in-grid Pn REMOVED` grammar - pills LIVE on the drill grid now. Data comes from a year-scoped `sc_day_metadata` query so actual first/last dates per period are known. Pill text = words (Kevin's amendment); tested on TBJ - FL July 13 densest cell (PROJECTED green + 3-row meal stack + P8 STARTS pill; fits legibly). AAA season squish diagnosis: dense MiLB tiles overflowed the ~35-40px portrait width with `OPP + 6:35 ET + optional DH` = up to 13 chars; grid + aspect-ratio 1/1 kept tile geometry uniform but content overflowed inside. Fix: strip trailing TZ suffix (` ET` / ` CT` / ` MT` / ` PT`) from cell times on every season variant (MLB + AAA + PDCO); TZ context added via a `Times local · {tz}` legend note. Row 3 layout waste (September alone + 2 empty tiles in the 3-col × 3-row grid) flagged as diagnosis-only finding; restructuring the grid is a bigger architectural call than the amendment scopes. Period-scope pill inclusion (STL - FL P8) flagged as a visible divergence from the amendment's "month scope only" wording; kept for signal, PR body posts for Kevin's veto. Contact-sheet law: every print PR paragraph-per-sheet in the PR body, not verdict-target subsetting.
- **2026-07-13 (PDC drill park)** - PDC + PDCO drill PDF (`scope=month` + `scope=period`) PARKED behind Coming Soon on the SC release pending the wall-poster redesign. Menu gate: `src/app/service-calendar/season/ExportControl.js` greys the drill item + adds a `COMING SOON` tag when `has_homestand_schedule === false` (PDC + PDCO); MLB + AAA drill items stay live. Route gate (defense-in-depth): `/api/service-calendar/print` returns 404 on `scope=month|period` when the account's `has_homestand_schedule=false`, with a JSON body pointer to `docs/design/PDC_PRINT_REDESIGN.md`. Season PDF, Ops Calendar PDF, and Excel exports for these accounts stay live. All PDF scopes for MLB (CIN - OH, STL - MO, TXR - TX - H/V) + AAA (CIN - KY, TBJ - NY) drill sheets stay live. Sheet code + `gen-all-pdfs.mjs` entries UNCHANGED - proofs remain generatable internally for the design side. Redesign arc + six open rulings + resume procedure in `docs/design/PDC_PRINT_REDESIGN.md`. Un-gate when Kevin approves the redesign build: remove the menu disabled state + drop the route 404.
- **2026-07-14 (Bug B fix + schedule-truth doctrine)** - Fixed the "vanishing schedule days" class on the SCREEN Month drill for schedule-bearing accounts. Root cause: `loadMonthDataPostgres` built `days[]` from `sc_daily_revenue` view rows only, no schedule-truth fallback; getaway AWAY dates + missing-projection HOME days + PDCO overlay dates without projections all silently dropped. 27+ MLB fee + 12 AAA (TBJ - NY) + 24 PDCO (STL - FL) dates affected in 2026. Fix: `addMissingScheduleDates` helper in `src/lib/dataStore/serviceCalendar.js:29` called by both `loadMonthDataPostgres` (:775-796) and `loadYearSummaryPostgres` (~:1210) with the UNION of homestand + overlay dates. Widened from the original fee-only scope after Kevin's ruling that MLB API schedule is calendar truth for every schedule-bearing account and projections overlay it, never define it (docs/modules/SERVICE_CALENDAR.md "Schedule truth hierarchy"). Unit tests: `scripts/content/__tests__/sc-fee-fallback.test.mjs` (32/32 green). No new deps, no schema changes. Counter-only data migration `docs/migrations/sc-18-day-metadata-counter-patches.sql` fixes 5 rows of `sc_day_metadata.game_type` misclassification surfaced by Part 3 (CIN 5/29 + 8/20 + TXR H/V 3/26/28/29 opening series) - runs alongside the code fix, migration-gated. Bug A (transient month-swap) parked as pre-release known-issue with reproduction checklist in `SC_STATUS.md` per Kevin's hard rule (no fix without named file:line mechanism).
- **2026-07-14 (sc-19 DATE_DRIFT safe subset)** - Part 4 bidirectional audit surfaced 24 DATE_DRIFT rows (`sc_homestand_schedule.service_date` != current API date after postponements) + 5 real AAA MISSING_IN_DB pks. Kevin's ruling split the reconciliation: rows whose target date is FREE + no operator actuals -> UPDATE now; two-games-one-date collisions -> wait for Option A; rows carrying operator actuals -> GUARDED, need ops review. Collision + actuals probe (`scripts/_probe_sc_19_collision_check.mjs`) classified the 26 rows into 1 SAFE_NOW, 13 WAIT_OPTION_A, 12 GUARDED. Migration `docs/migrations/sc-19-date-drift-safe-subset.sql` applies the one SAFE_NOW row (STL - MO pk=823042 2026-06-25 -> 2026-07-23, the Cardinals HOME vs ARI that Part 1's one-way walk missed). The 12 GUARDED rows are all AAA + FSL per-meal accounts where every service day carries actuals (5-6 rows per date); the guard rightly stopped silent date rewrites. The 13 WAIT_OPTION_A + 2 AAA MISSING_IN_DB DH-game-2 rows are documented in the migration body for the Option A follow-up PR (post-cron ship per Kevin's sequencing).
- **2026-07-14 (Stage 1 schedule-drift watchdog)** - Nightly cron `/api/cron/schedule-drift` at `0 11 * * *` (06:00 ET) diffs the MLB / MiLB Stats API against `sc_homestand_schedule` for the 8 schedule-bearing accounts and posts alerts to `#service-calendar` via `SLACK_SC_WEBHOOK_URL`. **Zero writes to schedule / projection / metadata tables** - Stage 1 is detect-and-notify only per Kevin's ruling; applying changes stays the manual extract -> migration flow. Pure differ (`src/lib/scheduleDrift.js`, 21 unit tests). `KNOWN_ISSUES` suppression seeded from Part 4's unreconciled population (25 pks) - groups those into a single "known gaps" one-liner instead of ~50 alarms night one. Noise discipline: post ONLY on drift, cron failure, or the Monday heartbeat. Dry-run flag `?dry=1` returns the intended payload as JSON without posting. Rate: 8 API calls + 8 DB reads per night. Stages 2/3 (auto-draft migration PRs, ON CONFLICT auto-apply) PARKED to 2027 review. sc-19b GUARDED reconcile turned out empty under the actuals-stay ruling - every previously-GUARDED row ALSO has a target-date collision, so all 12 join the Option A backlog and appear in `KNOWN_ISSUES` instead of a migration.
- **2026-07-14 (schedule-truth arc CLOSED)** - Trigger: Kevin's TXR "100% wrong" red flag. Outcome: 2 bugs surfaced + fixed (Bug B rendering mechanism named + fixed in #430; sc-18 counter patches + sc-19 date-drift SAFE_NOW applied in Studio same day), 1 parked with a trap (Bug A transient month-swap parked with reproduction checklist in `SC_STATUS.md`), doctrine enshrined (`docs/modules/SERVICE_CALENDAR.md` "Schedule truth hierarchy" + "Drift detection (Stage 1, LIVE)"), watchdog live (#432 → nightly Slack notify + Monday heartbeat). PR trail #430 → #431 → #432 all merged 2026-07-14. Full arc synthesis - timeline, PR trail, migration record, 7-item rulings ledger, 7 open rulings carried forward, `KNOWN_ISSUES` prune pointer for Option A - in `docs/audits/SC_SCHEDULE_TRUTH_AUDIT_2026-07.md` "ARC CLOSEOUT" section.
