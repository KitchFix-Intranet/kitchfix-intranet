...
**Append-only notes ledger.** Day notes persist to `sc_day_note_entries` (one row per authored note; author derived server-side from the session, never accepted from client input). The `sc-add-note` action appends; DayDetail renders the ledger client-side. The dormant `sc_day_metadata.day_notes` column was backfilled into the ledger with `author = NULL` in Postgres; the UI renders `"—"` as the display placeholder (typographic no-data convention). Landed via #367 (SC-079); schema is `docs/migrations/sc-9-day-note-entries.sql`. **F1:** DayDetail renders this ledger as the unified **Activity** view - notes merged client-side with the `sc_daily_actuals_history` edit trail (also server-derived author, populated by the `sc_daily_actuals_audit` BEFORE UPDATE trigger). Both sources unchanged; only the render composes them.
