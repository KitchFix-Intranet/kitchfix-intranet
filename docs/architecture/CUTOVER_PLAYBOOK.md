...
- **2026-06-03 (evening, II)** - STL-MO line-item recovery. A photo-upload guard had silently suppressed extraction for 133 submissions over 7 weeks; the gap was invisible because no check reconciled capture-rate. Backfilled all 133 via the live extraction path, gated by the arithmetic check; held 6 overcount Kuna invoices from promotion. Reinforces the still-open action item: a daily reconciliation alarm (submissions-without-line-items + dual-write drift) would have caught both this and the local-env dual-write gap on day one.
