...
### A view's cost measured in aggregate says nothing about its cost under a filter
...
Fix landed in `purchasing-9` (expression index on `SUBSTRING(external_id, 1, 24)`) + `purchasing-10` (view rewrite to `NOT EXISTS` + `WITH label_to_account AS MATERIALIZED`). Post-fix: same TBJ - FL FYTD query = **172 ms**. But the wrong-measurement is what let a live 500 ship.

**Rule:** measure a view the way the route actually calls it - one route path, one WHERE clause, one EXPLAIN ANALYZE against production. If the route filters on a column, measure the view under that filter, not against the aggregate. An in-code probe that reproduces the view's logic isn't a substitute; the planner reads the SQL, not the intent.

**Fourth instance of the same shape** on this project: the PostgREST 1000-row cap (measured one page, extrapolated to all), the four-slot chart (measured one range, extrapolated to all), the walk at 16.5% (measured one filter value, extrapolated to all), and now this - measured the view in aggregate, extrapolated to filter-shape reads. The pattern: measure ONE context, assume ANOTHER. The fix is always the same: measure the exact shape production runs.
