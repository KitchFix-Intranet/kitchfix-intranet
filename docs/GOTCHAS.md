...
### SC rename traps - places the product identifies something by name string, not by ID
As of 2026-08-04 the SC admin surface does NOT ship a rename button for services, groups, or accounts, so none of the sites below are broken today. Each is a landmine armed for the day admin grows one. The list is what would break, in the order it would bite.

**The sharpest first: R3-5's Spring Training section matches members on the " - ST" name suffix.** `src/app/service-calendar/v2/entry/DayEntryV2.js:1222,1267`. `s.name.endsWith(" - ST")` decides which of a fee-account's services belongs to the Spring Training grouping. Rename a Spring Training service to drop the suffix and the section silently EMPTIES - the four ST services fall back to being loose rows inside their real MLB / MiLB groups. No error, no test failure, just an empty section header on STL - FL until an operator notices.

Other sites, ordered by consequence:

- **Group + service resolution on the price change path.** `src/app/api/service-calendar/route.js:1282` in the `sc-config-update` handler. `s.groupName === c.groupName && s.serviceName === c.serviceName` resolves the client-provided (group, service) pair to a serviceId. A rename of either side fails LOUDLY with `Service not found in config: <group> / <service>` and a 400, so admin sees the break rather than eating it. Least dangerous of the set because it errors.
- **MLB homestand-surface account gate.** `src/app/service-calendar/pilots.js:45-50` `MLB_HOMESTAND_SURFACE_ACCOUNTS`. A hardcoded `Set(["CIN - OH", "STL - MO", "TXR - TX - H", "TXR - TX - V"])` that decides whether an account gets the homestand rail + tracker + closeout surface. Rename an MLB account and the account silently reverts to the Season overview shape - operators lose homestand navigator with no signal.
- **Homestand derivation gate.** `src/app/service-calendar/season/homestandDerivation.js:74-79` `DERIVE_HOMESTANDS_ACCOUNTS`. Same 4-key set. Rename produces an empty homestand-block derivation; the SeasonStepper renders nothing. Independent from the pilot gate above and would need to be renamed in both places.
- **DayEntryV2 rollout allowlist.** `src/lib/flags.js:184-192` `ENTRY_V2_ACCOUNTS`. A per-account cutover set. Rename silently reverts the account to the legacy entry UI - no error, just an experience regression that would look like a browser cache issue.
- **Seed migration filters.** `docs/migrations/sc-1-service-calendar-schema.sql:44-54`, and other one-shot data migrations. `WHERE team_key IN ('CIN - AZ', ...)` and `WHERE team_key = 'STL - FL'` clauses seed `billing_model`, category flags, and phase overlays. Point-in-time by design and safe to leave, EXCEPT that renaming an account and then re-running a seed against the new key would skip the `billing_model IS NULL` guard silently - the new key gets a NULL billing_model and every downstream billing branch breaks. Migrations here are ledger, not runtime; the trap only fires on a re-seed after a rename.

Three separate sites key on the same 4-key MLB account set (pilots, derivation, and to a lesser extent the ENTRY_V2 allowlist). Any account rename that would ship needs to move all three synchronously; a rename PR that touches one without the others regresses the corresponding surface.

The ST-suffix match at R3-5 is the only site whose fix needs a schema change (an explicit `is_spring_training` flag on `sc_services`); the rest can be closed with a rename button that runs an in-transaction update across the account key everywhere it appears.
