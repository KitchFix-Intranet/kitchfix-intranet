...
### Parallel research agents produce exploration, not authority

Six agents assembled `ACCOUNT_REFERENCE.md` in parallel; two returned confabulated data - one an all-zeros department table (Section 3), one placeholder Rippling IDs like `DEPT-CIN-AZ-HLY` (Section 2). Both looked structurally correct. Neither was queried against live data.

**The rule:** fan-out is right for discovering what exists. Any value that will be treated as canonical gets generated by a single pass against live data with `[ran]` evidence. **A confabulated identifier in a reference document is worse than a gap**, because a gap gets questioned and a plausible wrong value does not - the reader trusts the marker column instead of the value.

**Realised on 2026-08-05.** For `ACCOUNT_REFERENCE.md`, Sections 1, 4, 5, 6 (exploration/collation) used research agents; Sections 2 and 3 (canonical identifiers) got single-pass generator scripts (`scripts/_gen_section2.mjs`, `scripts/_gen_section3.mjs`) that query live Postgres + Rippling. Re-run them when the map changes.
