---
id: REC-104
title: "Account Record - STL-FL (St. Louis Cardinals (Jupiter FL PDC))"
doc_class: REC
shelf: "Service Delivery & Client Accounts"
subshelf: "Account Records"
status: In Build
version: "0.1"
card_line: "Canonical per-account record for STL-FL: identity, contract, billing shape, operations, rulings, open items, history. Live prices via Price Book."
summary: "The canonical Account Record for STL-FL (St. Louis Cardinals (Jupiter FL PDC)). Identity and aliases crosswalk; contract pointer to CONTRACT_DIGEST_STL-FL; billing record (money shape, rate table, passthrough, ancillary, worked example); operations record; current rulings; open items with owners; and preserved history. Billing figures are specimens - live values resolve from the generated Price Book. Restricted."
keywords:
  - STL-FL
  - account record
  - billing
...
relationships:
  ...
  - { to: REF-125, type: references, from_section: "section 1 Contract - verbatim source-of-record" }
---

> Canonical record. Current-state above the fold; history preserved below (the History section). Primary key is the intranet account name. Reasoning/decisions journaled in LEDGER (repo-side provenance); verbatim contract terms in the Contract Digest (REF-125).
...
- **Verbatim source-of-record**: the Contract Digest (REF-125).
...
- **The base agreement** (STL-MO's `2025-27 Food Services Agreement`) is the "Agreement" this amends - defined terms carry over (the Provenance & Attribution section). *digest §A*
...
- **SF TAX: NON-TAXABLE** (bills TAX 0.00) - invoice-confirmed (K300168343: $350,000, TAX $0.00). Kevin ruled §U (A-6); reconfirmed Josh/Lessard Slack §Y. **This is the Cardinals' asserted legal position, not a settled exemption** - "defensible," pushed by the Cardinals' lawyers; if the state challenged it and won, **the Cardinals would bear the tax liability**. *§U, §Y, §3.1 evidence*
  - **CONTRAST with CIN-OH** (SF taxed 7.80%). Per-account SF-tax attribute - STL-FL exempt, CIN-OH taxed. *§X*
...
## Passthrough / reimbursables (billed separately from the $1.4M, at cost)
...
- **Food/snacks/beverages/packaging/supplies budget = $900,000** (§2.a.ii), billed **bi-monthly with receipts** at exact cost (no markup); savings revert to the Cardinals. OUT of the $1.4M fee and OUT of SC per-meal scope. *digest §B.7, Kevin*
  - **Passthrough invoices NOT sampled for STL-FL.** Structure mirrors STL-MO's reimbursables (single-line lump, tax-zero, at-cost). Confirm when an STL-FL reimbursable invoice is obtained. *gap - §X*
...
## 2027 work-stoppage lock-in (forward-looking, captured for completeness)
...
|---|---|---|
| A-6 | CLOSED | §U, §Y |
| $24,500 credit | CLOSED | §W, §X |
| Fee figure | CLOSED | §W |
| Upkeep ownership | CLOSED | Kevin |
| Full passthrough | CLOSED | Kevin |
| Reimbursable tax | CLOSED | §X |

**Detail**

- **A-6** - STL-FL SF is **NON-TAXABLE** (bills TAX 0.00). Confirmed by invoice + Kevin ruling + Josh/Lessard Slack. Cardinals' asserted legal position; Cardinals bear the liability risk.
- **$24,500 credit** - The "prior payment" on K300168343 = a 2025 tax-matter credit, applied as a "payment" line against the SF installment. Credits offset future installments (not separate refunds).
- **Fee figure** - 2026 Florida Services Fee = $1,400,000 (finance + invoice confirmed); $900K passthrough + $30K upkeep separate.
- **Upkeep ownership** - The $30K upkeep envelope is a **Cardinals-reimbursed passthrough** (KitchFix spends, Cardinals reimburse) - NOT KitchFix-borne.
- **Full passthrough** - ALL food/snacks/beverages at STL-FL are reimbursable passthrough (the $900K stream).
- **Reimbursable tax** - Reimbursables bill tax-zero (tax paid to vendors already; avoids double-tax). General rule.
# Open Items

| Item | Status | Owner | Blocking cert? |
|---|---|---|---|
| $900K passthrough invoice | PENDING (accounting) | accounting | No |
| Finance-doc credit note placement | OPEN (minor) | Sebastian | No |
| Fun Money allocation | OPEN (planned) | Kevin | No |
| STL-FL MiLB Snack price missing | OPEN (minor) | Kevin | No |

**Detail**

- **$900K passthrough invoice** - STL-FL reimbursable invoice not sampled. Structure inferred from STL-MO (single-line lump, tax-zero, at-cost). Confirm when obtained - the STL-FL $900K is the largest passthrough of the 11.
- **Finance-doc credit note placement** - The $24,500 credit note in the finance workbook (§W) sits on installment 3, but the credit actually applied on installment 4 (per invoice K300168343). Off-by-one in the finance doc; not a billing error. Flag for correction.
- **Fun Money allocation** - $25,000 "Fun Money" (not contractually defined) - `is_non_revenue`, planned to move to the Fun Money Tracker.
- **STL-FL MiLB Snack price missing** - Snack projection price blank in the SC - reconcile (flat-fee, so $0 anyway, but the service row is incomplete).
...
- **Contract facts**: the Contract Digest (REF-125) (the Amendment, verbatim) + the Contract Digest (REF-125) (the base agreement it amends).
...
- **Money shape**: the Money Model (REF-140) (flat_fee, fee-no-dollar variant).
- **Rulings**: LEDGER (repo-side provenance) §U (A-6 SF non-taxable), §W (fee), §X (credit).
