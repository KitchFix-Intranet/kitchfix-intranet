...
|---|---|---|
| Model | CLOSED | §Z |
| Pricing authority | CLOSED | §Z |
| Two streams | CLOSED | §Z |
| Revenue location | CLOSED (current) / FUTURE (migration) | §Z |
| Off-menu pricing | CLOSED | §Z |
| `PFS (Away)` | CLOSED | §X, §Y |

**Detail**

- **Model** - TXR-V = per-team catering (each visiting team = independent client, billed directly, per-series). NOT flat-fee/per-meal-contract.
- **Pricing authority** - The 2026 menu (`THE VITAL PARTNER.pdf` + addendum) IS the price source. No signed Price Review; no per-team pricing.
- **Two streams** - Carve-in (G&G/snacks/coffee) = $0, covered by TXR-H $604K. Catering = separate direct-bill revenue. Keep distinct.
- **Revenue location** - CURRENT: standalone Season Tracker → P&L 2400.1, SC operational-only. FUTURE: migrate tracker into SC (Kevin roadmap, not yet done).
- **Off-menu pricing** - One-off/off-menu requests priced case-by-case (internal). Bill export must allow ad-hoc line items.
- **`PFS (Away)`** - Confirmed activity code for visiting-team catering (mirror of `PFS (Home)`). PFS = Performance Food Service.
# Open Items

| Item | Status | Owner | Blocking cert? |
|---|---|---|---|
| Cubs invoice spot-check | PENDING | Kevin | No |
| Season Tracker → SC migration | FUTURE (roadmap) | Kevin | No |
| Paperwork gap (no catering SOW) | OPEN (informational) | Kevin | No |
| SC models buffet for TXR-V | OPEN (SC cleanup) | Kevin | No |
| Contact roster review | RECURRING | Kevin | No |
| Unpriced new items | OPEN (minor) | menu team | No |

**Detail**

- **Cubs invoice spot-check** - Kevin sending the Cubs TXR-V invoice; will confirm the per-team pattern (direct-bill, menu prices, 4% CC) holds identically across a second team. Yankees invoice already confirms the model.
- **Season Tracker → SC migration** - Currently standalone; Kevin plans to fold the visiting-catering revenue tracker into the SC. Future change - not modeled as in-SC today.
- **Paperwork gap (no catering SOW)** - The à-la-carte catering has no contractual basis in the Rangers agreement (only the G&G/snacks/coffee carve-in is contracted). It's an off-contract commercial arrangement with each visiting team. Not a defect to fix - just documented.
- **SC models buffet for TXR-V** - Prior open item #25: SC mirrors TXR-H's full buffet service list on TXR-V, but the *contract* scope for V is G&G/snacks/coffee only. The buffet catering is real (Stream B) but off-contract - decide whether the SC keeps buffet rows for operational tracking or deletes them.
- **Contact roster review** - Roster is point-in-time; review periodically + full refresh at year-start. Minor Tab1/Tab2 discrepancies (CLE, SD, TOR) to reconcile.
- **Unpriced new items** - Southern Grits, Biscuits & Gravy, Pataya Bowl have no price on the addendum.
