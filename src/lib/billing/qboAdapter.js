...
//
// ─── HARD FENCES (this PR only; owner ruling 2026-08-11) ──────────
//
// 1. `ALLOWED_CUSTOMER_IDS` is a frozen SET of exactly one QBO
//    customer id: 22463 ("ZZ TEST - KitchFix Intranet"). ANY payload
//    whose CustomerRef.value is not in this set is REFUSED at the
//    top of postInvoiceDraft, BEFORE the network. This is enforced
//    by constant, not by convention - so a code path that "forgot"
//    the fence has no way to reach a real customer id from this PR.
//
//    The wire-in from finalize (src/lib/scWeekFinalize.js
//    runFinalizeEffects) DOES build payloads for real customers.
//    Those payloads hit the fence and land in the ledger as
//    'failed' rows with a NotAllowlistedError; the finalize
//    transitions to 'push_failed' and N2 fires. No real customer
//    can be POSTed to from this PR's code paths. Rollout removes
//    the fence in a follow-up PR after owner + Sebastian sign off.
//
...
// ─── Fences ───────────────────────────────────────────────────────
export const ALLOWED_CUSTOMER_IDS = Object.freeze(new Set(["22463"]));
...
export class NotAllowlistedError extends Error {
  constructor(customerId) {
    super(
      `qboAdapter: CustomerRef.value=${JSON.stringify(customerId)} is not in ALLOWED_CUSTOMER_IDS. PR-C is fenced to 22463 only.`
    );
 ...
 * @param {Object}   ctx
 * @param {boolean}  ctx.isTest      When true: apply test markers,
 *                                   route to customer 22463.
...
export async function postInvoiceDraft(payload, ctx) {
  ...
  // ─── Fence: hard allow-list ───────────────────────────────────
  const customerId = outgoing?.CustomerRef?.value;
  if (!ALLOWED_CUSTOMER_IDS.has(String(customerId))) {
    ...
    const ledgerRowId = await writeLedgerRow(supa, {
      ...
      status:             "failed",
      attempt,
      error:              `NotAllowlistedError: CustomerRef.value=${customerId} not in ALLOWED_CUSTOMER_IDS. PR-C fence rejected.`,
      is_test:            isTest,
      created_by:         ctx.createdBy,
    });
    const err = new NotAllowlistedError(customerId);
