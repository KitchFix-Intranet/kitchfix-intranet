...
async function resolveRange(supa, { scope, year, period, month, accountKey }) {
  ...
  // period scope
  // Live storage format: sc_day_metadata.period holds BARE numeric
  // strings ("1".."13"), not "P1".."P13". The pre-2026-07-09 code
  // queried against "P{n}" and matched zero rows on every period
  // export - shipped hotfix strips any leading P before the .eq().
  // Input stays tolerant of both "7" and "P7" so the CTA can hand off
  // whichever it happens to hold.
  if (period == null) throw new Error("[scWorkbook] period param required for scope=period");
  const pNum = String(period).replace(/^[Pp]/, "");
  ...
  if (!data?.length) throw new Error(`[scWorkbook] period ${pNum} not found for ${accountKey} in ${year}`);
  const first = data[0].service_date;
  const last  = data[data.length - 1].service_date;
  return { first, last, label: `Period ${pNum}`, shortLabel: `Period${pNum}`, weekLabels: [] };
}
...
function buildSummarySheet(workbook, ctx, notes) {
  ...
  // Explicit America/Chicago timestamp + " CT" suffix so the operator
  // reads the wall-clock time of the export in their own timezone
  // (Vercel runs in UTC; the Date object rendered with a bare numFmt
  // showed UTC). Stored as a string, not a Date - the "CT" suffix is
  // part of the value, not the format.
  sheet.getCell("B4").value = generatedAt.toLocaleString("en-US", {
...
function buildRevenueBlock(sheet, startRow, ctx) {
  ...
  // Whole-dollar discipline (matches the app: SC-057 rounded the toast
  // to whole dollars, fmt$ default is decimals=0, and the KPI reads
  // are integer-summed anyway). Rounding here kills the float tail on
  // the year path where per-month sums accumulate small residues.
  ...
  if (revenueExternal) {
    ...
    // Missing-fee honesty (F1 hotfix). SC_MONEY_MODEL says the fee
    // schedule is the SOLE source of Service Fee data - absence is
    // a data gap to surface, never to paper over. The all-in KPI
    // row below stays pointer-only until a fee row exists.
    sheet.getCell(`A${row}`).value = sfLabel;
    sheet.getCell(`A${row}`).font = { bold: true };
    sheet.mergeCells(`B${row}:D${row}`);
    const cell = sheet.getCell(`B${row}`);
    cell.value = `not on file for ${year} - add via SC Admin → Fees to enable the all-in KPI`;
    cell.font = { color: { argb: MUTED_INK }, italic: true };
    row++;
  }
  // (fee shapes without a fee row is the unusual case and stays a
  // silent skip - the SERVICE block above already carries the fee
  // reference for those accounts.)
...
function countRow(sheet, row, label, proj, actual, variance) {
  ...
  // Explicit sign on the variance cell (+22,178 / -22,178 / 0). Excel's
  // bare '#,##0' shows negatives with a leading '-' but positives as
  // bare - the sign is the whole point of a variance column, so force
  // both.
  sheet.getCell(`D${row}`).value = variance; sheet.getCell(`D${row}`).numFmt = '+#,##0;-#,##0;0';
  sheet.getCell(`D${row}`).font = { color: { argb: variance >= 0 ? DELTA_OVER : DELTA_UNDER }, bold: true };
}
