...
// 2026-07-11: swapped from Lucide's `send` icon (a paper-plane-shaped
// arrow triangle - read as "arrow" not "plane" per Kevin's review) to
// Lucide's `plane` icon path (a proper airplane silhouette with visible
// wings + tail). Rendered STROKED with round joins so the wing/tail
// details survive at 13px on tiles - filling this path renders as a
// chunky abstract shape at small sizes and loses the "plane" read. The
// stroked outline matches the shape convention of other tile glyphs
// (--sc icon set: `stroke="currentColor"` strokeWidth 1.75). Sized
// independently at 13px on tiles and 12px in the legend.
