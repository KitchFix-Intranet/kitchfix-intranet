...
function ServiceCalendarInner({ showToast, session, heroImage, firstName, isDev = false }) {
  ...
  const accountDropdown = isAdminView ? (
    <div className="sc-header-admin-label">
      {/* PR-N audit R2 B1 (2026-08-21): dropped to caption scale
          + removed "· all accounts" suffix. Once the accounts rail
          and catalog header both carry the account context, the
          suffix is redundant. The chip is a mode indicator, not
          an action - caption size fits the role. */}
      <span className="sc-admin-mode-chip sc-admin-mode-chip--sm">
