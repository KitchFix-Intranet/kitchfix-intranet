...
function renderPerMeal(content, status) {
  ...
  //
  // sc-17b (2026-07-11): guard the short-circuit on !opponent. TBJ - FL
  ...
  // revenue lines below (matches the sc-16 milb-branch fix).
  if (!meals && !opponent) {
    return (
  ...
  return (
    <div className="sc-daysq-mid sc-daysq-mid--stack">
      {/* sc-17b (2026-07-11): informational overlay for TBJ - FL
          ...
          row have opponent + dayNight null and these two rows
          disappear - the render collapses to the pre-sc-17b shape
          byte-for-byte. */}
      {opponent && (
