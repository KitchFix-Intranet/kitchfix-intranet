...
// Consolidation PR 1 (2026-09-22) - aggregation extracted to
// aggregate.js so both this page and the new LaborLedger on the
// Overview page consume the same math. VERBATIM lift per Kevin's
// three guards; no behavioural change here.
import {
  workerLabel,
...
// workerLabel moved to ./lib/aggregate.js (Consolidation PR 1,
// 2026-09-22). Re-imported at the top of this file. No behavioural
// change - the function body is byte-identical there.
...
export default function KpiLaborPage() {
  ...
  const filteredActuals = useMemo(
    () => buildFilteredActuals(data?.actuals, selectedWorkers),
  ...
  // excludedFromRollup helper moved into buildWeekAggregates.
  ...
  const weekAggregates = useMemo(
    () => buildWeekAggregates(filteredActuals, aggregateExcludedSet, data?.board?.weeks),
    [filteredActuals, aggregateExcludedSet, data?.board?.weeks],
  );
  const weeksInRange = weekAggregates.length; // canonical week count

  const grouped = useMemo(
    () => buildGrouped(weekAggregates, rangeSelectionEarly, data?.board),
  ...
  const workerRoster = useMemo(
    () => buildWorkerRoster(data?.actuals, data?.workers, redact),
    [data, redact],
  );
  ...
  const grand = useMemo(
    () => buildGrand(grouped, today, data?.board?.avg_rate),
  ...
  const resolvedPreset = useMemo(
    () => computeResolvedPreset(lastPreset, start, end, today, data?.account_periods),
