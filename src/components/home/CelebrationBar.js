...
export default function CelebrationBar({ celebrations, standard }) {
  ...
  return (
    <div className="kf-home-section">
      <div className="kf-home-section-header">
        <div className="kf-home-section-title">
        ...
        {hasCelebrations && celebrations.length > 3 && (
          <div className="kf-cel-controls">
            <button className="kf-cel-btn" onClick={() => scrollPicket("left")}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="15 18 9 12 15 6" /></svg>
            </button>
            <button className="kf-cel-btn" onClick={() => scrollPicket("right")}>
      ...
      {hasCelebrations ? (
        <div className="kf-cel-scroll" id="celebration-container">
          {celebrations.map((c, i) => {
            const isBirthday = c.type?.toLowerCase() === "birthday";
            const iconBg = isBirthday ? "rgba(124, 58, 237, 0.1)" : "rgba(16, 185, 129, 0.1)";
            const iconColor = isBirthday ? "#7c3aed" : "#10b981";
            const borderColor = isBirthday ? "#7c3aed" : "#10b981";

            return (
              <div key={i} className="kf-cel-tile" style={{ borderLeftColor: borderColor }}>
                <div className="kf-cel-icon" style={{ background: iconBg }}>
                  {isBirthday ? (
                ...
                <div className="kf-cel-text">
                  <div className="kf-cel-name">{c.firstName}</div>
                  <div className="kf-cel-label">{c.subLabel}</div>
                </div>
              </div>
            );
          })}
          <div className="kf-cel-tile kf-cel-tile--end">
            <div className="kf-cel-icon" style={{ background: "#f1f5f9" }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2">
                <circle cx="12" cy="12" r="10" /><line x1="12" y1="16" x2="12" y2="12" /><line x1="12" y1="8" x2="12.01" y2="8" />
              </svg>
            </div>
            <div className="kf-cel-text">
              <div className="kf-cel-name">Check Back...</div>
              <div className="kf-cel-label">Tomorrow for more</div>
            </div>
          </div>
        </div>
      ) : (
        <div className="kf-motivation">
