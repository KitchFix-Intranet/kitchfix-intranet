...
const TOOLS_CONFIG = [
  ...
  { id: "avy",   name: "Coming Soon",  color: "#94a3b8", url: null, icon: "tag", disabled: true },
    ...
    <div className="kf-home-section">
      ...
      <div className="kf-tools-grid">
        {sorted.map((tool) => {
          ...
          return (
            <div key={tool.id} className="kf-tool-wrap">
              <a
href={tool.url || "#"}
                target={tool.url ? "_blank" : undefined}
                rel={tool.url ? "noopener noreferrer" : undefined}
                className={`kf-tool-tile${isPinned ? " kf-tool-tile--pinned" : ""}${tool.disabled ? " kf-tool-tile--disabled" : ""}`}
              ...
              >
                ...
                <span className="kf-tool-name">{tool.name}</span>
              </a>
              <button
                className={`kf-tool-star${isPinned ? " kf-tool-star--active" : ""}`}
