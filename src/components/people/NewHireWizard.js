      ...
      <div className="pp-view" style={{ animation: "pp-slideUp 0.4s ease" }}>
        <div className="pp-card pp-card--form">
          <div className="pp-success-view">
            ...
            <h2 className="pp-card-title">{form.isEdit ? "Correction Resubmitted!" : "Team Reinforced!"}</h2>
            <p className="pp-card-desc">{form.firstName} has been queued for onboarding.</p>
            {/* Fix #4: Back to Home is primary, Start Another is secondary */}
            <div style={{ display: "flex", flexDirection: "column", gap: 12, marginTop: 24, width: "100%", maxWidth: 280, margin: "24px auto 0" }}>
              ...
              <button className="pp-btn pp-btn--ghost" onClick={() => resetForm(true)}>Start Another Hire</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="pp-view" style={{ animation: "pp-slideUp 0.4s ease" }}>
      <div className="pp-card pp-card--form">
        ...
        <div className="pp-form-content">
          ...
          {step === 1 && (
            <>
              ...
              <div className="pp-pill-group">
                <button type="button" className={`pp-pill-option${form.isRehire !== "Yes" ? " pp-pill-option--active" : ""}`} onClick={() => update("isRehire", "No")}>New Employee</button>
                <button type="button" className={`pp-pill-option${form.isRehire === "Yes" ? " pp-pill-option--active" : ""}`} onClick={() => update("isRehire", "Yes")}>Returning</button>
              </div>
            </>
          )}
                ...
                onChange={(e) => {
                  ...
                  if (val === "Other") {
                    ...
                    if (MGMT_ROLES.includes(val)) {
                      ...
                      update("needsCard", "Yes");
                      update("needsEmail", "Yes");
                      update("needsLaptop", "Yes");
                      update("needsCell", "Yes");
                    } else {
                      update("payType", "Hourly");
                    }
                    salaryConfirmed.current = false;
                  }
                }}
          ...
          {step === 3 && (
            <>
              <h3 className="pp-card-title">Let&apos;s talk pay & tools.</h3>
              <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                <div className="pp-pill-group">
                  <button type="button" className={`pp-pill-option${!isSalary ? " pp-pill-option--active" : ""}`} onClick={() => { update("payType", "Hourly"); salaryConfirmed.current = false; }}>Hourly</button>
                  <button type="button" className={`pp-pill-option${isSalary ? " pp-pill-option--active" : ""}`} onClick={() => { update("payType", "Salary"); salaryConfirmed.current = false; update("isFullTime", true); update("needsCard", "Yes"); update("needsEmail", "Yes"); update("needsLaptop", "Yes"); update("needsCell", "Yes"); }}>Salary</button>
                </div>
              </div>
        ...
        <div className="pp-modal-overlay" onClick={() => setShowReview(false)}>
          <div className="pp-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="pp-review-body">
              {/* Fix #1: Formatter.toDate on startDate | Fix #2: purple circle edit buttons */}
              {[
                ["Candidate", `${form.firstName} ${form.lastName}`, 1],
                ["Role", form.jobTitle, 2],
                ["Manager", form.manager, 2],
                ["Start Date", Formatter.toDate(form.startDate), 3],
                ["Comp", `${Formatter.toMoney(form.payRate)} (${form.payType})`, 3],
            ...
            <div className="pp-modal-footer">
              ...
              {/* Fix #3: subtle send icon on confirm button */}
