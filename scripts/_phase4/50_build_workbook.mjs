// Build the Phase 4 workbook. Overwrites the existing xlsx.
// Phase 4 is a weight-recovery pass over Phase 3c. Every dollar figure is
// byte-identical to Phase 3c (dollar invariance check enforced upstream).
// Weight-related cells (Protein Mix, Weight & Coverage, Account Comparison
// lbs_per_meal, Item Master parsed_weight_source_p4) updated with recovered
// rows. NEW SHEET at end: "Phase 4 Recovery Log".
//
// Sheets (order):
//   1. Summary                       (adds real reconciliation numbers)
//   2. Methodology                   (adds Phase 3c method + plausibility bands)
//   3. Account Comparison            (STL-FL per-meal cells suppressed)
//   4. Category Mix                  (unchanged)
//   5. Category Totals               (unchanged)
//   6. Premium vs Commodity          (unchanged - byte-identical to 3b)
//   7. Prefabricated vs Scratch      (unchanged - byte-identical to 3b; adds one neutral line)
//   8. Frozen vs Fresh               (unchanged)
//   9. Weight & Coverage             (2 category $/lb cells suppressed by IQR band)
//  10. Protein Mix                   (TBJ pork + STL seafood recomputed via item 5 row exclusions)
//  11. Duplicate Item Families       (50 spread cells suppressed)
//  12. Top Items                     (unchanged)
//  13. Shared Basket                 (unchanged)
//  14. Price Variance                (unchanged)
//  15. Price Drift                   (unchanged)
//  16. Vendor Concentration          (unchanged)
//  17. Pareto                        (unchanged)
//  18. Order Cadence                 (unchanged)
//  19. Unique Items                  (unchanged)
//  20. Item Master                   (unchanged)
//  21. Needs Review                  (adds Phase 3c suppression lists)
//  22. Reconciliation                (unchanged)
//  23. Phase 3b Fix Log              (unchanged)
//  24. Phase 3c Change Log           (NEW - what changed vs Phase 3b)

import fs from "node:fs";
import ExcelJS from "exceljs";
import { ACCOUNTS, MONTHS } from "/Users/kevinfietek/dev/purchase-discovery-2026-08-12/scripts_phase3c/_common3b.mjs";
import { P as P4, PUBLICATION_THRESHOLD_PCT } from "./_common4.mjs";

const ANALYSIS_4 = P4.P4_ANALYSIS;
const CHANGE_LOG_4 = P4.P4_CHANGE_LOG;
const CHANGE_LOG_3C = "/Users/kevinfietek/dev/purchase-discovery-2026-08-12/scripts_phase3c/_change_log.json";
const WORKBOOK = "/Users/kevinfietek/dev/purchase-discovery-2026-08-12/PURCHASE_ANALYSIS_2026_MAY_JUL.xlsx";

const A = JSON.parse(fs.readFileSync(ANALYSIS_4, "utf8"));
const CL3C = JSON.parse(fs.readFileSync(CHANGE_LOG_3C, "utf8"));
const CL4 = JSON.parse(fs.readFileSync(CHANGE_LOG_4, "utf8"));

const wb = new ExcelJS.Workbook();
wb.creator = "Phase 4 purchasing analysis - protein weight coverage recovery";
wb.created = new Date();
...
// ============================================================================
{
  ...
  // Phase 3c item 2 - print real reconciliation numbers per account
  s.getCell(r, 1).value = "Reconciliation - line-item sum vs invoice header sum, per-account window variance (DOLLAR SET)";
  s.getRow(r).font = { bold: true, size: 12 };
  r++;
  const recHdr = s.getRow(r);
  recHdr.values = ["Account", "Variance %"];
  styleHeaderRow(recHdr);
  r++;
  for (const acct of ACCOUNTS) {
    const v = A.reconciliation[acct].window.variance_pct;
    const row = s.getRow(r);
  ...
  r++;
  s.getCell(r++, 1).value = "Real per-account window variances shown above (item 2, Phase 3c). Full monthly breakdown on the Reconciliation sheet.";
  r++;
...
// ============================================================================
{
  ...
  s.getCell(r++, 1).value = "Meals denominator (Fix 2 in Phase 3b; Phase 3c suppresses all STL-FL per-meal figures)";
  s.getRow(r - 1).font = { bold: true };
  ...
  s.getCell(r++, 1).value = "  Phase 3c item 3: ALL STL-FL per-meal and per-lb-per-meal cells replaced with 'meals data insufficient' because the STL meals denominator is too weak (June 0 actuals, July 5 rows / 164 meals). TBR-FL and TBJ-FL per-meal figures unchanged.";
  s.getCell(r++, 1).value = "  Any monthly cell with computed $/meal > 3x window average = suppressed (red), logged on Needs Review.";
  r++;

  s.getCell(r++, 1).value = "Plausibility bands per category (Phase 3c item 4)";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = "  Method: For each category present in Weight & Coverage, collect the dollars_per_lb values across the three accounts. Compute Q1, Q3, IQR = Q3 - Q1. Band = [max(0, Q1 - 1.5*IQR), Q3 + 1.5*IQR]. Any category-account $/lb outside its band is replaced with 'review - outside expected range' and listed on Needs Review with the computed value, coverage, and lbs.";
  s.getCell(r++, 1).value = "  Applied uniformly - no per-account or per-cell tuning. With n=3 datapoints per category the rule is strict; it fires only when a value is separated from Q1 and Q3 by more than 1.5x the interquartile spread. Categories with n<2 datapoints pass through unflagged.";
  const bandsHdr = s.getRow(r);
  bandsHdr.values = ["Category", "n", "Q1", "Q3", "Band low", "Band high"];
  styleHeaderRow(bandsHdr);
  r++;
  for (const [cat, band] of Object.entries(CL3C.item4_plausibility_band_flags.bands)) {
    const row = s.getRow(r);
    row.getCell(1).value = cat;
    row.getCell(2).value = band.n;
    row.getCell(3).value = band.q1 ?? "";
    if (typeof band.q1 === "number") row.getCell(3).numFmt = "$#,##0.00";
    row.getCell(4).value = band.q3 ?? "";
    if (typeof band.q3 === "number") row.getCell(4).numFmt = "$#,##0.00";
    row.getCell(5).value = band.band_low ?? band.method;
    if (typeof band.band_low === "number") row.getCell(5).numFmt = "$#,##0.00";
    row.getCell(6).value = band.band_high ?? "";
    if (typeof band.band_high === "number") row.getCell(6).numFmt = "$#,##0.00";
    r++;
  }
  r++;

  s.getCell(r++, 1).value = "Phase 3c item 5 (bounded diagnosis on two named cells)";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = "  Scope: TBJ-FL pork $/lb and STL-FL seafood $/lb ONLY. No other cells re-computed. See Phase 3c Change Log sheet + Needs Review sheet for exact row exclusions and before/after values.";
  r++;

  s.getCell(r++, 1).value = "Phase 3c item 6 (premium/prefabricated axes are independent)";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = "  Quality and preparation are independent axes; an item can be both premium and prefabricated (a pre-portioned Choice steak is both). Present the observation; do not interpret it. Premium and prefabricated figures are byte-identical to Phase 3b.";
  r++;
...
function classificationSheetTwoBasis(axisKey, sheetName, description) {
  ...
  if (axisKey === "quality" || axisKey === "preparation") {
    ...
    r++;
    // Phase 3c item 6 - neutral explanatory line
    s.getCell(r++, 1).value = "Note: Quality and preparation are independent axes; an item can be both premium and prefabricated (a pre-portioned Choice steak is both). Present the observation; do not interpret it.";
    s.getCell(r - 1, 1).alignment = { wrapText: true };
    s.getRow(r - 1).height = 30;
  }
  freezeHeaders(s, 3);
}
...
// ============================================================================
{
  ...
  for (const acct of ACCOUNTS) {
    ...
    for (const c of cats) {
      ...
      if (typeof c.dollars_per_lb === "number") row.getCell(9).numFmt = "$#,##0.00";
      else if (typeof c.dollars_per_lb === "string") row.getCell(9).font = { italic: true };
    ...
    r++;
    // Phase 4 annotation: what changed from Phase 3c
    const p4 = A.weight[acct];
    if (p4._phase4_before) {
      const note = s.getRow(r);
      note.getCell(1).value = "  Phase 4 recovery";
      note.getCell(1).font = { italic: true };
      note.getCell(1).alignment = { horizontal: "left" };
      const before = p4._phase4_before;
      note.getCell(2).value = `+${p4._phase4_added_rows} rows / +$${p4._phase4_added_food_spend} spend / +${p4._phase4_added_food_lbs} lb; coverage ${before.coverage_food_spend_pct}% -> ${p4.coverage_food_spend_pct}%`;
      s.mergeCells(r, 2, r, 9);
      note.getCell(2).font = { italic: true };
      note.getCell(2).alignment = { horizontal: "left", wrapText: true };
      r++;
    }
    r++;
  }
...
// ============================================================================
{
  ...
  for (const acct of ACCOUNTS) {
    ...
    for (const b of pm.by_type) {
      ...
      row.getCell(5).numFmt = "#,##0.0";
      row.getCell(6).value = b.pct_of_protein_lbs != null && typeof b.pct_of_protein_lbs === "number" ? b.pct_of_protein_lbs : "insufficient coverage";
      if (typeof b.pct_of_protein_lbs === "number") row.getCell(6).numFmt = '0.0"%"';
      row.getCell(7).value = b.coverage_spend_pct;
      row.getCell(7).numFmt = '0.0"%"';
      // Phase 4: publish $/lb only when coverage >= 35% and not plant_or_egg
      if (b._phase4_suppressed) {
        row.getCell(8).value = b._phase4_pub_dollars_per_lb;
        row.getCell(8).font = { italic: true };
        row.getCell(8).fill = SUPPRESSED_FILL;
      } else {
      ...
      // Phase 4 annotation - what changed from Phase 3c
      const notes = [];
      if (b._phase3c_note) notes.push(`P3c item 5 recompute (${b._item5_row_exclusion_count} rows excluded)`);
      if (b._phase4_recovered_rows) {
        const before = b._phase4_before;
        notes.push(`P4 recovery: +${b._phase4_recovered_rows} rows / +${b._phase4_recovered_lbs} lb; coverage ${before.coverage_spend_pct}% -> ${b.coverage_spend_pct}%; $/lb ${before.dollars_per_lb} -> ${b.dollars_per_lb}`);
      }
      if (notes.length > 0) {
        row.getCell(9).value = notes.join(" | ");
        row.getCell(9).font = { italic: true };
  ...
  r++;
  s.getCell(r++, 1).value = "Note (Phase 3c/4): TOTAL row updated in Phase 4 to include recovered lbs. Individual dollars_per_lb cells published where coverage >= 35% publication threshold; otherwise suppressed with reason. plant_or_egg category always suppressed (eggs sold by count, not weight - see Phase 4 Recovery Log for Path D recommendation).";
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  s.getRow(r - 1).height = 40;
  freezeHeaders(s, 3);
}
...
// ============================================================================
{
  ...
  setColWidths(s, [40, 20, 14, 14, 22, 8, 14, 14, 8, 34, 14, 8, 34, 14, 8, 34, 28]);
  let r = 1;
  s.getCell(r, 1).value = "Every distinct (vendor, description) pair in DOLLAR SET with three-axis classification + Fix-4 basis + beverage-reclassified flag. Last column: Phase 4 weight recovery tag when the SKU had rows recovered via Path A/B/C.";
  s.getCell(r, 1).font = { bold: true };
  r += 2;
  const header = s.getRow(r);
  header.values = ["Description", "Vendor", "Category", "Basis", "Accounts", "Orders", "Spend", "Quality", "Q conf", "Quality reason", "Preparation", "P conf", "Preparation reason", "Storage", "S conf", "Storage reason", "P4 recovery tag"];
  styleHeaderRow(header);
  r++;
  for (const it of A.item_master) {
    ...
    if (it._phase4_recovery_tag) {
      row.getCell(17).value = `${it._phase4_recovery_tag} (path ${it._phase4_recovery_path}, ${it._phase4_recovery_rows} rows)`;
      row.getCell(17).font = { italic: true };
    }
    r++;
  }
  freezeHeaders(s, 3);
...
// ============================================================================
{
  ...
  // ============================================================================
  // Phase 3c suppression lists
  // ============================================================================
  s.getCell(r, 1).value = "Phase 3c item 1 - Duplicate Family spread cells suppressed";
  s.getRow(r).font = { bold: true };
  r++;
  s.getCell(r++, 1).value = `Rule: suppress $/lb spread where family confidence < 70 OR spread > 3x. ${CL3C.item1_duplicate_family_spread_suppressions_summary.families_suppressed} families suppressed, covering $${CL3C.item1_duplicate_family_spread_suppressions_summary.total_spend_covered}.`;
  const item1Hdr = s.getRow(r);
  item1Hdr.values = ["Family", "Category", "Confidence", "Original spread ratio", "SKU count", "Spend", "Reason", ""];
  styleHeaderRow(item1Hdr);
  r++;
  for (const s1 of CL3C.item1_duplicate_family_spread_suppressions) {
    const row = s.getRow(r);
    row.getCell(1).value = s1.family;
    row.getCell(2).value = s1.category;
    row.getCell(3).value = s1.confidence;
    row.getCell(4).value = s1.original_spread_ratio;
    row.getCell(4).numFmt = '0.00"x"';
    row.getCell(5).value = s1.sku_count;
    row.getCell(6).value = s1.spend;
    row.getCell(6).numFmt = "$#,##0.00";
    row.getCell(7).value = s1.reason;
    r++;
  }
  r += 2;

  s.getCell(r, 1).value = "Phase 3c item 3 - STL-FL per-meal cells suppressed";
  s.getRow(r).font = { bold: true };
  r++;
  s.getCell(r++, 1).value = "Rule: STL-FL meals denominator too weak to support per-meal figures (June 0 actuals, July 5 rows / 164 meals). All 10 STL-FL per-meal cells (3 months x 2 basis + 4 window) replaced with 'meals data insufficient'.";
  const item3Hdr = s.getRow(r);
  item3Hdr.values = ["Scope", "Account", "Month / window", "Metric", "Original value", "", "", ""];
  styleHeaderRow(item3Hdr);
  r++;
  for (const s3 of CL3C.item3_stl_per_meal_suppressions) {
    const row = s.getRow(r);
    row.getCell(1).value = s3.scope;
    row.getCell(2).value = s3.account;
    row.getCell(3).value = s3.month || "window";
    row.getCell(4).value = s3.metric;
    row.getCell(5).value = s3.original_value;
    if (typeof s3.original_value === "number") row.getCell(5).numFmt = "$#,##0.00";
    r++;
  }
  r += 2;

  s.getCell(r, 1).value = "Phase 3c item 4 - Category $/lb cells outside IQR band";
  s.getRow(r).font = { bold: true };
  r++;
  s.getCell(r++, 1).value = "Rule: per-category IQR-based band [max(0, Q1 - 1.5*IQR), Q3 + 1.5*IQR] on the population of category $/lb across three accounts. Cells outside their band suppressed and listed here.";
  const item4Hdr = s.getRow(r);
  item4Hdr.values = ["Category", "Account", "Computed $/lb", "Band low", "Band high", "Coverage % spend", "Weight lbs", ""];
  styleHeaderRow(item4Hdr);
  r++;
  for (const s4 of CL3C.item4_plausibility_band_flags.flagged_cells) {
    const row = s.getRow(r);
    row.getCell(1).value = s4.category;
    row.getCell(2).value = s4.account;
    row.getCell(3).value = s4.computed_dollars_per_lb;
    row.getCell(3).numFmt = "$#,##0.00";
    row.getCell(4).value = s4.band[0];
    row.getCell(4).numFmt = "$#,##0.00";
    row.getCell(5).value = s4.band[1];
    row.getCell(5).numFmt = "$#,##0.00";
    row.getCell(6).value = s4.coverage_spend_pct;
    row.getCell(6).numFmt = '0.0"%"';
    row.getCell(7).value = s4.weight_lbs;
    row.getCell(7).numFmt = "#,##0.0";
    r++;
  }
  r += 2;

  s.getCell(r, 1).value = "Phase 3c item 5 - Bounded diagnosis (two named cells only)";
  s.getRow(r).font = { bold: true };
  r++;
  for (const [cellName, diag] of Object.entries(CL3C.item5_bounded_diagnosis)) {
    s.getCell(r, 1).value = cellName;
    s.getCell(r, 1).font = { bold: true };
    r++;
    s.getCell(r++, 1).value = `  Outcome: ${diag.outcome}`;
    s.getCell(r++, 1).value = `  Root cause: ${diag.root_cause}`;
    s.getCell(r - 1, 1).alignment = { wrapText: true };
    s.getRow(r - 1).height = 45;
    s.getCell(r++, 1).value = `  Rows excluded: ${diag.excluded_row_count}   Spend covered: $${diag.excluded_row_spend}   Weight excluded (pre): ${diag.excluded_row_lbs_pre} lb`;
    s.getCell(r++, 1).value = `  BEFORE: rows=${diag.before.weight_rows} lbs=${diag.before.weight_lbs} spend=$${diag.before.weight_spend} $/lb=$${diag.before.dollars_per_lb} coverage=${diag.before.coverage_spend_pct}%`;
    s.getCell(r++, 1).value = `  AFTER:  rows=${diag.after.weight_rows} lbs=${diag.after.weight_lbs} spend=$${diag.after.weight_spend} $/lb=$${diag.after.dollars_per_lb} coverage=${diag.after.coverage_spend_pct}%`;
    r++;
    // Excluded rows detail
    const eHdr = s.getRow(r);
    eHdr.values = ["Excluded row description", "Pack size", "Quantity", "Extended $", "Weight lb (pre)", "Implied $/lb (pre)", "Weight method", ""];
    styleHeaderRow(eHdr);
    r++;
    for (const er of diag.excluded_rows) {
      const row = s.getRow(r);
      row.getCell(1).value = er.description;
      row.getCell(2).value = er.pack_size;
      row.getCell(3).value = er.quantity;
      row.getCell(4).value = er.extended_price;
      row.getCell(4).numFmt = "$#,##0.00";
      row.getCell(5).value = er.effective_weight_lb_pre;
      row.getCell(5).numFmt = "#,##0.00";
      row.getCell(6).value = er.implied_dpp_pre;
      row.getCell(6).numFmt = "$#,##0.00";
      row.getCell(7).value = er.weight_method;
      r++;
    }
    r += 2;
  }
  ...
  // ============================================================================
  // Phase 4 - protein_mix cells suppressed by 35% publication threshold
  // ============================================================================
  s.getCell(r, 1).value = "Phase 4 - protein_mix $/lb cells suppressed by 35% publication threshold";
  s.getRow(r).font = { bold: true };
  r++;
  s.getCell(r++, 1).value = "Rule: any protein-type $/lb cell with weight-coverage < 35% of that type's DOLLAR SET spend is suppressed. Publication threshold is FIXED at 35% - not tuned per cell.";
  if (CL4.suppressions_from_35pct_threshold.length === 0) {
    s.getCell(r++, 1).value = "  (none - all cells cleared threshold)";
  } else {
    const sHdr = s.getRow(r);
    sHdr.values = ["Account", "Cell", "Coverage % spend", "$/lb (would-have)", "", "", "", ""];
    styleHeaderRow(sHdr);
    r++;
    for (const su of CL4.suppressions_from_35pct_threshold) {
      const row = s.getRow(r);
      row.getCell(1).value = su.account;
      row.getCell(2).value = su.cell;
      row.getCell(3).value = su.coverage_spend_pct;
      row.getCell(3).numFmt = '0.0"%"';
      row.getCell(4).value = su.dollars_per_lb_would_have_been;
      if (typeof su.dollars_per_lb_would_have_been === "number") row.getCell(4).numFmt = "$#,##0.00";
      r++;
    }
  }
  r += 2;
  s.getCell(r, 1).value = "Phase 4 - protein_mix cells that CLEARED the 35% threshold thanks to recovery";
  s.getRow(r).font = { bold: true };
  r++;
  if (CL4.cells_cleared_35pct.length === 0) {
    s.getCell(r++, 1).value = "  (none)";
  } else {
    const cHdr = s.getRow(r);
    cHdr.values = ["Account", "Cell", "Coverage before", "Coverage after", "", "", "", ""];
    styleHeaderRow(cHdr);
    r++;
    for (const c of CL4.cells_cleared_35pct) {
      const row = s.getRow(r);
      row.getCell(1).value = c.account;
      row.getCell(2).value = c.cell;
      row.getCell(3).value = c.before_pct;
      row.getCell(3).numFmt = '0.0"%"';
      row.getCell(4).value = c.after_pct;
      row.getCell(4).numFmt = '0.0"%"';
      r++;
    }
  }
...
// ============================================================================
// Sheet 24: Phase 3c Change Log (NEW - final tightening pass)
// ============================================================================
{
  const s = wb.addWorksheet("Phase 3c Change Log");
  setColWidths(s, [30, 40, 25, 25, 25]);
  let r = 1;
  s.getCell(r++, 1).value = "Phase 3c Change Log - narrow, final tightening pass over Phase 3b";
  s.getCell(r - 1, 1).font = { bold: true, size: 14 };
  s.getCell(r++, 1).value = `Build date: ${CL3C.meta.build_date}`;
  s.getCell(r++, 1).value = `Rule: ${CL3C.meta.rule}`;
  s.getCell(r++, 1).value = "Every figure not listed below is byte-identical to Phase 3b.";
  r++;

  s.getCell(r++, 1).value = "Item 1 - Duplicate Family spread suppressions";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = `Families suppressed: ${CL3C.item1_duplicate_family_spread_suppressions_summary.families_suppressed} (spend covered: $${CL3C.item1_duplicate_family_spread_suppressions_summary.total_spend_covered})`;
  s.getCell(r++, 1).value = "Rule: family confidence < 70 OR intra-family $/lb spread > 3x. Value replaced with 'not comparable'. Row kept (SKU count + spend visible).";
  r++;

  s.getCell(r++, 1).value = "Item 2 - Reconciliation numbers";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = "Previously stated as '<8%' aggregate. Now shows three real per-account window variances:";
  for (const [acct, v] of Object.entries(CL3C.item2_reconciliation.actual_values)) {
    const row = s.getRow(r);
    row.getCell(1).value = `  ${acct}`;
    row.getCell(2).value = v;
    row.getCell(2).numFmt = '0.0"%"';
  ...
  s.getCell(r++, 1).value = "Item 3 - STL-FL per-meal suppressions";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = `Cells suppressed in workbook: ${CL3C.item3_summary.workbook_cells_suppressed}`;
  s.getCell(r++, 1).value = `Cells suppressed in CEO one-pager: ${CL3C.item3_summary.ceo_one_pager_cells_suppressed}`;
  s.getCell(r++, 1).value = CL3C.item3_summary.note;
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  const item3Hdr = s.getRow(r);
  item3Hdr.values = ["Scope", "Month or window", "Metric", "Original value"];
  styleHeaderRow(item3Hdr);
  r++;
  for (const s3 of CL3C.item3_stl_per_meal_suppressions) {
    const row = s.getRow(r);
    row.getCell(1).value = s3.scope;
    row.getCell(2).value = s3.month || "window";
    row.getCell(3).value = s3.metric;
    row.getCell(4).value = s3.original_value;
    if (typeof s3.original_value === "number") row.getCell(4).numFmt = "$#,##0.00";
    r++;
  }
  r++;

  s.getCell(r++, 1).value = "Item 4 - Plausibility band suppressions";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = CL3C.item4_plausibility_band_flags.method;
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  s.getRow(r - 1).height = 50;
  const bhdr = s.getRow(r);
  bhdr.values = ["Category", "n", "Q1 $/lb", "Q3 $/lb", "Band low", "Band high"];
  styleHeaderRow(bhdr);
  r++;
  for (const [cat, band] of Object.entries(CL3C.item4_plausibility_band_flags.bands)) {
    const row = s.getRow(r);
  ...
  r++;
  s.getCell(r++, 1).value = `Cells flagged and suppressed: ${CL3C.item4_plausibility_band_flags.flagged_cells.length}`;
  const fhdr = s.getRow(r);
  fhdr.values = ["Category", "Account", "Computed $/lb", "Band [low, high]"];
  styleHeaderRow(fhdr);
  r++;
  for (const f of CL3C.item4_plausibility_band_flags.flagged_cells) {
    const row = s.getRow(r);
    row.getCell(1).value = f.category;
    row.getCell(2).value = f.account;
    row.getCell(3).value = f.computed_dollars_per_lb;
    row.getCell(3).numFmt = "$#,##0.00";
    row.getCell(4).value = `[$${f.band[0]}, $${f.band[1]}]`;
    r++;
  }
  r += 2;

  s.getCell(r++, 1).value = "Item 5 - Bounded diagnosis (two named cells)";
  s.getRow(r - 1).font = { bold: true };
  for (const [cellName, diag] of Object.entries(CL3C.item5_bounded_diagnosis)) {
    s.getCell(r++, 1).value = cellName;
    s.getCell(r - 1, 1).font = { bold: true };
    ...
    s.getCell(r - 1, 1).alignment = { wrapText: true };
    s.getRow(r - 1).height = 45;
    s.getCell(r++, 1).value = `Excluded ${diag.excluded_row_count} rows / $${diag.excluded_row_spend} / ${diag.excluded_row_lbs_pre} lb (pre-exclusion)`;
    s.getCell(r++, 1).value = `Before: $/lb = $${diag.before.dollars_per_lb} (${diag.before.weight_rows} rows, ${diag.before.weight_lbs} lb, coverage ${diag.before.coverage_spend_pct}%)`;
    s.getCell(r++, 1).value = `After:  $/lb = $${diag.after.dollars_per_lb} (${diag.after.weight_rows} rows, ${diag.after.weight_lbs} lb, coverage ${diag.after.coverage_spend_pct}%)`;
    r++;
  }
  r++;

  s.getCell(r++, 1).value = "Item 6 - Premium / prefabricated";
  s.getRow(r - 1).font = { bold: true };
  s.getCell(r++, 1).value = CL3C.item6_premium_prefabricated.note;
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  s.getRow(r - 1).height = 30;
  s.getCell(r++, 1).value = "Quality and preparation are independent axes; an item can be both premium and prefabricated (a pre-portioned Choice steak is both). Present the observation; do not interpret it.";
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  s.getRow(r - 1).height = 30;
  r++;
...
// ============================================================================
// Sheet 25: Phase 4 Recovery Log (NEW)
// ============================================================================
{
  const s = wb.addWorksheet("Phase 4 Recovery Log");
  setColWidths(s, [40, 28, 18, 16, 16, 16, 16]);
  let r = 1;
  s.getCell(r, 1).value = "Phase 4 - protein weight coverage recovery";
  s.getRow(r).font = { bold: true, size: 14 };
  r++;
  s.getCell(r, 1).value = "Goal: MORE real data traced to a documented source (donor row, invoice image, vendor catalog). 35% publication threshold FIXED. No dollar figures changed. No re-classification.";
  s.getCell(r, 1).alignment = { wrapText: true };
  s.getRow(r).height = 40;
  r += 2;

  // Diagnostic
  s.getCell(r, 1).value = "TBJ-FL protein diagnostic (before Phase 4)";
  s.getRow(r).font = { bold: true };
  r++;
  const diag = JSON.parse(fs.readFileSync("/Users/kevinfietek/dev/purchase-discovery-2026-08-12/scripts_phase4/_diagnostic.json", "utf8"));
  const dHdr = s.getRow(r);
  dHdr.values = ["Protein type", "Rows", "Spend", "Resolved rows", "Resolved spend", "Coverage % spend", ""];
  styleHeaderRow(dHdr);
  r++;
  for (const [type, tv] of Object.entries(diag["TBJ-FL"].per_type)) {
    const row = s.getRow(r);
    row.getCell(1).value = type;
    row.getCell(2).value = tv.rows;
    row.getCell(3).value = tv.spend;
    row.getCell(3).numFmt = "$#,##0.00";
    row.getCell(4).value = tv.resolved_rows;
    row.getCell(5).value = tv.resolved_spend;
    row.getCell(5).numFmt = "$#,##0.00";
    row.getCell(6).value = tv.coverage_spend_pct;
    row.getCell(6).numFmt = '0.0"%"';
    r++;
  }
  r += 2;
  ...
  // Recovery per Path
  const REC = JSON.parse(fs.readFileSync("/Users/kevinfietek/dev/purchase-discovery-2026-08-12/scripts_phase4/_recovered_rows.json", "utf8"));
  s.getCell(r, 1).value = "Recovery summary by Path";
  s.getRow(r).font = { bold: true };
  r++;
  const pHdr = s.getRow(r);
  pHdr.values = ["Path", "Description", "Rows recovered", "Spend recovered", "Lbs recovered", "", ""];
  styleHeaderRow(pHdr);
  r++;
  const paths = [
    ["A", "Cross-account item_number weight borrowing (safest, from clean same-SKU sibling rows anywhere in dataset)", REC.summary.by_path.A],
    ["B", "Bacon invoice reads (11 TBJ-FL Sysco invoices carrying 16 Phase-3c-excluded bacon rows)", REC.summary.by_path.B],
    ["C", "Top-80% TBJ-FL unresolved-protein-SKU invoice reads (39 invoices)", REC.summary.by_path.C],
  ];
  for (const [pkey, desc, v] of paths) {
    const row = s.getRow(r);
    row.getCell(1).value = `Path ${pkey}`;
    row.getCell(1).font = { bold: true };
    row.getCell(2).value = desc;
    row.getCell(2).alignment = { wrapText: true };
    row.getCell(3).value = v.rows;
    row.getCell(4).value = v.spend;
    row.getCell(4).numFmt = "$#,##0.00";
    row.getCell(5).value = v.lbs;
    row.getCell(5).numFmt = "#,##0.0";
    r++;
    s.getRow(r - 1).height = 32;
  }
  r += 2;

  s.getCell(r, 1).value = "Recovery per account per protein type";
  s.getRow(r).font = { bold: true };
  r++;
  const ppHdr = s.getRow(r);
  ppHdr.values = ["Account", "Protein type", "Rows", "Spend", "Lbs", "", ""];
  styleHeaderRow(ppHdr);
  r++;
  for (const [a, byType] of Object.entries(REC.summary.per_account_per_protein_type)) {
  ...
  s.getCell(r, 1).value = "Before / after coverage per account per protein type (Phase 3c -> Phase 4)";
  s.getRow(r).font = { bold: true };
  r++;
  const baHdr = s.getRow(r);
  baHdr.values = ["Account", "Type", "Before coverage %", "After coverage %", "Before $/lb", "After $/lb", "Publish?"];
  styleHeaderRow(baHdr);
  r++;
  for (const upd of CL4.protein_mix_updates) {
    const row = s.getRow(r);
    row.getCell(1).value = upd.account;
    row.getCell(2).value = upd.type;
    row.getCell(3).value = upd.before.coverage_spend_pct;
    row.getCell(3).numFmt = '0.0"%"';
    row.getCell(4).value = upd.after.coverage_spend_pct;
    row.getCell(4).numFmt = '0.0"%"';
    row.getCell(5).value = upd.before.dollars_per_lb;
    if (typeof upd.before.dollars_per_lb === "number") row.getCell(5).numFmt = "$#,##0.00";
    row.getCell(6).value = upd.after.dollars_per_lb;
    if (typeof upd.after.dollars_per_lb === "number") row.getCell(6).numFmt = "$#,##0.00";
    row.getCell(7).value = upd.after.coverage_spend_pct >= 35 ? "YES (>=35%)" : "no (<35%)";
    if (upd.after.coverage_spend_pct >= 35) row.getCell(7).font = { bold: true };
    r++;
  }
  r += 2;

  s.getCell(r, 1).value = "Cells CLEARED the 35% publication threshold (were suppressed, now published)";
  s.getRow(r).font = { bold: true };
  r++;
  if (CL4.cells_cleared_35pct.length === 0) {
    s.getCell(r++, 1).value = "  (none)";
    ...
    const cHdr = s.getRow(r);
    cHdr.values = ["Account", "Cell", "Before %", "After %", "", "", ""];
    styleHeaderRow(cHdr);
    r++;
    for (const c of CL4.cells_cleared_35pct) {
      const row = s.getRow(r);
  ...
  s.getCell(r, 1).value = "Path D - plant_or_egg recommendation";
  s.getRow(r).font = { bold: true };
  r++;
  s.getCell(r++, 1).value = "Category plant_or_egg (eggs, tofu, seitan, tempeh): 41 rows / $4,623.57 across three accounts. Eggs sold by count not weight. Liquid egg products carry pack sizes but the count-based rows dominate. Not resolvable to lbs.";
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  s.getRow(r - 1).height = 30;
  s.getCell(r++, 1).value = "RECOMMENDATION (Kevin decides): compute lbs-per-meal denominators EXCLUDING plant_or_egg category. Do NOT count as missing coverage. Phase 4 has flagged plant_or_egg cells as suppressed in Protein Mix with reason 'eggs sold by count'.";
  s.getCell(r - 1, 1).alignment = { wrapText: true };
  s.getRow(r - 1).height = 40;
  r += 2;

  s.getCell(r, 1).value = "Out-of-scope defects logged (NOT fixed in Phase 4)";
  s.getRow(r).font = { bold: true };
  r++;
  const oosLines = [
    "Sysco pack-size OCR 'fused-slash' shape: pack codes like '410 LB' (=4/10 LB=40 lb/case), '115LB' (=1/15 LB=15 lb/case), '182#AVG' (catch-weight avg) commonly OCR without the slash - affects ~500+ Sysco lines dataset-wide. Bacon subset fixed in Phase 4 Path B; broader dataset NOT fixed. Recommend future extractor prompt refinement + parser rule to expand these patterns.",
    "Gordon Food Service pack codes '6x3 CO' (6 cases x 3 units, weight on separate CASE lines) not consistently parsed - vision reads pack but not the CASE-line weights. Some GFS invoices carry TOTAL WEIGHT lines; parser doesn't ingest them.",
    "Vendor `item_number` field frequently null on Cheney/Sysco/GFS lines (see diagnostic: many rows have `item#=none` even though the invoice shows one). Path A cross-account borrow depends on item_number; missing item_number caps recovery ceiling.",
    "TBJ-FL beef 25 rows / $6.3K carry ep_qty_up_mismatch with NO weight_line_value AND NO pack_size AND NO item_number - Phase 4 cannot recover without another Anthropic re-OCR pass on those specific invoices.",
    "Vendor catalog reference (order guides) not present in intranet - Path C used invoice images. A vendor-catalog Sheets/PG table would eliminate need for per-invoice image reads on repeated SKUs.",
  ];
  for (const l of oosLines) {
    const row = s.getRow(r);
    row.getCell(1).value = "  - " + l;
    row.getCell(1).alignment = { wrapText: true };
    s.mergeCells(r, 1, r, 7);
    s.getRow(r).height = 45;
    r++;
  }
  r += 2;

  s.getCell(r, 1).value = "Dollar invariance check";
  s.getRow(r).font = { bold: true };
  r++;
  s.getCell(r++, 1).value = `Dollar-figure invariance: ${CL4.dollar_invariance_check.pass ? "PASS" : "FAIL"} (${CL4.dollar_invariance_check.issues.length} issues)`;
  if (!CL4.dollar_invariance_check.pass) {
    for (const iss of CL4.dollar_invariance_check.issues) {
      s.getCell(r++, 1).value = `  ${iss.path}: 3c=${iss.phase3c} 4=${iss.phase4}`;
    }
  }
...
console.log("[4-wb] wrote", WORKBOOK);
