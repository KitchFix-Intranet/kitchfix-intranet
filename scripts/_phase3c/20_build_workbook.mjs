// Build the Phase 3c workbook. Overwrites the existing xlsx.
// Phase 3c is a narrow suppression-only pass over Phase 3b. Every figure not
// listed in the Phase 3c change log is byte-identical to Phase 3b. New sheet:
// "Phase 3c Change Log" (last position).
//
// Sheets (order):
...
import { ACCOUNTS, MONTHS } from "./_common3b.mjs";

const ANALYSIS_3C = "/Users/kevinfietek/dev/purchase-discovery-2026-08-12/scripts_phase3c/_analysis3c.json";
...
const A = JSON.parse(fs.readFileSync(ANALYSIS_3C, "utf8"));
const CL3C = JSON.parse(fs.readFileSync(CHANGE_LOG_3C, "utf8"));

const wb = new ExcelJS.Workbook();
wb.creator = "Phase 3c purchasing analysis - final tightening pass";
wb.created = new Date();
...
// ============================================================================
{
  ...
  for (const acct of ACCOUNTS) {
    ...
    for (const b of pm.by_type) {
      ...
      // Phase 3c item 5 - flag rows where dollars_per_lb was recomputed via row exclusion
      if (b._phase3c_note) {
        row.getCell(9).value = `item 5 recompute (${b._item5_row_exclusion_count} rows excluded; see Change Log)`;
  ...
  r++;
  s.getCell(r++, 1).value = "Note (Phase 3c): total_protein_lbs and lbs_coverage_spend_pct on the TOTAL row are Phase 3b values and include the item-5-excluded rows. Only the individual dollars_per_lb for the two named cells (TBJ-FL pork, STL-FL seafood) has been recomputed. See Phase 3c Change Log sheet for full before/after.";
  s.getCell(r - 1, 1).alignment = { wrapText: true };
...
console.log("[3c-wb] wrote", WORKBOOK);
